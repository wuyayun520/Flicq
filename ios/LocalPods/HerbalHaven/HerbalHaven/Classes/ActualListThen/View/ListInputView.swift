...
fileprivate let str_sizeBottomName:[UInt8] = [0x4e,0x53,0x4e,0x59,0xd,0x48,0x54,0x49,0x4a,0x57,0x1f,0xe,0x5,0x4d,0x46,0x58,0x5,0x53,0x54,0x59,0x5,0x47,0x4a,0x4a,0x53,0x5,0x4e,0x52,0x55,0x51,0x4a,0x52,0x4a,0x53,0x59,0x4a,0x49]

fileprivate func titleShow(biology num: UInt8) -> UInt8 {
...
/*: "8075F5" :*/
fileprivate let str_viewMakeData:String = "from black bean image8075F5"

/*: "9A9B9C" :*/
fileprivate let str_greenTitle:[Character] = ["9","A","9","B","9","C"]

/*: "kOpacityAnimation" :*/
fileprivate let str_circleContent:[Character] = ["k","O","p","a","c","i","t","y","A","n","i","m","a"]
fileprivate let str_rowText:String = "keyon"

/*: "opacity" :*/
fileprivate let str_tableMakeText:String = "opacelli"
fileprivate let str_perspectiveName:String = "edit"

/*: "forwards" :*/
fileprivate let str_mediumTitle:String = "foflowwaflowds"

/*: "easeIn" :*/
fileprivate let str_linkDataName:String = "row vieweaseIn"

/*: "8C7AF8" :*/
fileprivate let str_matchTitleText:[Character] = ["8","C","7","A","F","8"]
...
class ListInputView: UIView {
    ...
    public init(frame: CGRect, codeNumber: Int = 6, labelTextColor: UIColor = UIColor.black, mainColor: UIColor = UIColor.orange, normalColor: UIColor = UIColor.gray, margin: CGFloat = 12.0) {
        ...
        self.margin = actualWidth(w: margin)
        //: self.setupSubviews()
        self.nameEditPremium()
        //: self.setupSubViewsConstraint()
        self.windowClickRestriction()
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: str_sizeBottomName.map{titleShow(biology: $0)}, encoding: .utf8)!)
    }
    ...
    lazy var textField: UITextField = {
        ...
        view.addTarget(self, action: #selector(blooper(_:)), for: .editingChanged)
        //: return view
        return view
        //: }()
    }()
...
extension ListInputView {
    ...
    func signDown() {
        ...
        for index in 0 ..< codeNumber {
            ...
            if index < verStr.count {
                ...
                layer.shadowColor = UIColor(hex: (String(str_viewMakeData.suffix(6))))!.withAlphaComponent(0.6).cgColor
            }
            //: else {
            else {
                //: label.text = ""
                ...
                if index == verStr.count {
                    ...
                    layer.shadowColor = UIColor(hex: (String(str_greenTitle)))!.withAlphaComponent(0.25).cgColor
    ...
    fileprivate func section(index: Int, hidden: Bool) {
        //: let line = shapeArray[index]
        let line = shapeArray[index]
        //: if hidden {
        if hidden {
            //: line.removeAnimation(forKey: "kOpacityAnimation")
            line.removeAnimation(forKey: (String(str_circleContent) + str_rowText.replacingOccurrences(of: "key", with: "ti")))
            //: } else {
        } else {
            //: line.add(opacityAnimation(), forKey: "kOpacityAnimation")
            line.add(equality(), forKey: (String(str_circleContent) + str_rowText.replacingOccurrences(of: "key", with: "ti")))
        }
    ...
    fileprivate func equality() -> CABasicAnimation {
        //: let animation = CABasicAnimation.init(keyPath: "opacity")
        let animation = CABasicAnimation(keyPath: (str_tableMakeText.replacingOccurrences(of: "cell", with: "c") + str_perspectiveName.replacingOccurrences(of: "edit", with: "ty")))
        ...
        animation.fillMode = CAMediaTimingFillMode(rawValue: (str_mediumTitle.replacingOccurrences(of: "flow", with: "r")))
        //: animation.timingFunction = CAMediaTimingFunction.init(name: CAMediaTimingFunctionName.init(rawValue: "easeIn"))
        animation.timingFunction = CAMediaTimingFunction(name: CAMediaTimingFunctionName(rawValue: (String(str_linkDataName.suffix(6)))))
        //: return animation
        return animation
    }
}
...
extension ListInputView {
    ...
    private func nameEditPremium() {
        ...
        for index in 0 ..< codeNumber {
            ...
            line.fillColor = UIColor(hex: (String(str_matchTitleText)))?.cgColor
    ...
    private func windowClickRestriction() {}
