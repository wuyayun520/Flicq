...
fileprivate let str_sortData:[UInt8] = [0x64,0x65,0x74,0x6e,0x65,0x6d,0x65,0x6c,0x70,0x6d,0x69,0x20,0x6e,0x65,0x65,0x62,0x20,0x74,0x6f,0x6e,0x20,0x73,0x61,0x68,0x20,0x29,0x3a,0x72,0x65,0x64,0x6f,0x63,0x28,0x74,0x69,0x6e,0x69]

/*: "icon_chat_translate_nor" :*/
fileprivate let str_picContent:[Character] = ["i","c","o","n","_","c","h","a","t","_","t","r","a","n","s","l","a"]
fileprivate let str_whiteKeyWithValue:[Character] = ["t","e","_","n","o","r"]

/*: "icon_chat_translate_pre" :*/
fileprivate let str_valueModelViewName:String = "angle type input priceicon_"
fileprivate let str_cornerSystemValue:String = "dayns"
fileprivate let str_rangeReasonContent:String = "rcenter"

/*: "icon_chat_miss" :*/
fileprivate let str_modelModeCardName:String = "icon_reply list false file height"
fileprivate let str_layerValue:String = "mibuttonbutton"

/*: "icon_chat_yes" :*/
fileprivate let str_hiddenData:String = "gift view player evaluate returnicon_"
...
class GroupMessageCell: LatchkeyMessageCell {
    //: var transBlock: (() -> Void)?
    var transBlock: (() -> Void)?
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: str_sortData.reversed(), encoding: .utf8)!)
    }
    ...
    lazy var transBtn: UIButton = {
        ...
        btn.setImage(UIImage.markTag(name: (String(str_picContent) + String(str_whiteKeyWithValue))), for: .normal)
        //: btn.setImage(UIImage.markTag(name: "icon_chat_translate_pre"), for: .highlighted)
        btn.setImage(UIImage.markTag(name: (String(str_valueModelViewName.suffix(5)) + "chat_t" + str_cornerSystemValue.replacingOccurrences(of: "day", with: "ra") + "late_p" + str_rangeReasonContent.replacingOccurrences(of: "center", with: "e"))), for: .highlighted)
        //: btn.addTarget(self, action: #selector(transBtnClick), for: .touchUpInside)
        btn.addTarget(self, action: #selector(playerSwitchline), for: .touchUpInside)
        //: return btn
        return btn
        //: }()
    }()
...
extension GroupMessageCell {
    ...
    override func fill(with data: TCommonCellData) {
        ...
        //: let maxWidth = AreaReactiveCompatible.getScreenWidth() - (15 + (self.cellData?.cellLayout.avatarSize.width ?? 45) + 10) * 2
        let maxWidth = AreaReactiveCompatible.r() - (15 + (self.cellData?.cellLayout.avatarSize.width ?? 45) + 10) * 2
        // 翻译赋值相关
        ...
        if textData.translateStatus == .MsgTranslate_translating {
            ...
            self.transStatusImagV.image = UIImage.markTag(name: (String(str_modelModeCardName.prefix(5)) + "chat_" + str_layerValue.replacingOccurrences(of: "button", with: "s")))
            //: } else if textData.translateStatus == .MsgTranslate_succeed {
        } else if textData.translateStatus == .MsgTranslate_succeed {
            //: self.transShowView.stopAnimating()
            self.transShowView.stopAnimating()
            //: self.transStatusImagV.image = UIImage.markTag(name: "icon_chat_yes")
            self.transStatusImagV.image = UIImage.markTag(name: (String(str_hiddenData.suffix(5)) + "chat_yes"))
        }
    ...
    func gold() {
        ...
        let longPressGr = UILongPressGestureRecognizer(target: self, action: #selector(destabilizationOf(tap:)))
        //: self.container.addGestureRecognizer(longPressGr)
        self.container.addGestureRecognizer(longPressGr)
    }

    //: @objc func LognCellMessageAction(tap: UILongPressGestureRecognizer) {
    @objc func destabilizationOf(tap: UILongPressGestureRecognizer) {
        //: if tap.state == .ended {
        if tap.state == .ended {
            //: return
            return
