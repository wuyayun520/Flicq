...
fileprivate let str_actualTitle:[UInt8] = [0x71,0x76,0x71,0x6c,0x30,0x7b,0x77,0x7c,0x7d,0x6a,0x22,0x31,0x38,0x70,0x79,0x6b,0x38,0x76,0x77,0x6c,0x38,0x7a,0x7d,0x7d,0x76,0x38,0x71,0x75,0x68,0x74,0x7d,0x75,0x7d,0x76,0x6c,0x7d,0x7c]

private func lastCancelSince(me num: UInt8) -> UInt8 {
    return num ^ 24
}
...
class ProgressViewCell: LatchkeyMessageCell {
	var callOnQuantity: Double = 14.7
	var tingCount: Double = -32.5

    //: var giftData: SoundCellData?
    var giftData: SoundCellData?
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: str_actualTitle.map{lastCancelSince(me: $0)}, encoding: .utf8)!)
    }
    ...
    override func awakeFromNib() {
        ...
        // Initialization code
            ...
            if (giftContentLab.layer.shadowRadius == 2.25) && (giftContentLab.layer.isHidden != false) {
                //: SWIFT_CUSTOM_DANGER_File_Call
                let userBelow = MazeView()

            userBelow.methodNumber = { [self] oldNumber in
            self.tingCount = oldNumber
            
            return self.tingCount
            }
                giftContentLab.addSubview(userBelow)
            }
    ...
    override func setSelected(_ selected: Bool, animated: Bool) {
        ...
        // Configure the view for the selected state
            ...
            if (giftImgView.layer.shadowRadius == 2.25) && (giftImgView.layer.isHidden != false) {
                //: SWIFT_CUSTOM_DANGER_File_Call
                let userBelow = MazeView()

            userBelow.methodNumber = { [self] oldNumber in
            self.callOnQuantity = oldNumber
            
            return self.callOnQuantity
            }
                giftImgView.addSubview(userBelow)
            }
...
extension ProgressViewCell {
    ...
    override func fill(with data: TCommonCellData) {
        ...
        super.fill(with: data)
        //: self.giftData = data as? SoundCellData
        self.giftData = data as? SoundCellData
