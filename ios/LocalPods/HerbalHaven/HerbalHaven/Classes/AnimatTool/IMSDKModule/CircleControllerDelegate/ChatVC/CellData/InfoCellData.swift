...
fileprivate let str_sinkTitle:[UInt8] = [0xf5,0xfa,0xf5,0x0,0xb4,0xef,0xfb,0xf0,0xf1,0xfe,0xc6,0xb5,0xac,0xf4,0xed,0xff,0xac,0xfa,0xfb,0x0,0xac,0xee,0xf1,0xf1,0xfa,0xac,0xf5,0xf9,0xfc,0xf8,0xf1,0xf9,0xf1,0xfa,0x0,0xf1,0xf0]

fileprivate func nameAdd(model num: UInt8) -> UInt8 {
...
/*: "[未知消息]" :*/
fileprivate let str_textObjectContent:[Character] = ["[","未","\u{77e5}","消","\u{606f}"]
fileprivate let str_signData:String = "]"

/*: "extra" :*/
fileprivate let str_indexMakeValue:String = "extrshare"

/*: "</at> " :*/
fileprivate let str_frameBlockText:String = "</at> selected start"

/*: "#0084FF" :*/
fileprivate let str_bottomActionTitle:[Character] = ["#","0","0","8","4","F","F"]

/*: "#FFEE4B" :*/
fileprivate let str_messageUserName:String = "#"
fileprivate let str_imageInfoName:String = "FFEE4visual"

/*: "<at>@[\\S\\s]+?</at>" :*/
fileprivate let str_unknownName:String = "extension let<at>@"
fileprivate let str_targetPushName:String = "s]+?view data leading"

/*: "jumpKey" :*/
fileprivate let str_layerValue:String = "make cell modeljumpKey"

/*: "user" :*/
fileprivate let str_reportName:[Character] = ["u","s","e","r"]

/*: "uid" :*/
fileprivate let str_hiddenValue:[UInt8] = [0x7e,0x62,0x6f]

private func resultButton(color num: UInt8) -> UInt8 {
    return num ^ 11
}

/*: "#16D073" :*/
fileprivate let str_makeData:[Character] = ["#","1","6","D","0","7","3"]

/*: "#F95644" :*/
fileprivate let str_partyText:[Character] = ["#","F","9","5","6","4","4"]
...
//: @objcMembers public class InfoCellData: TalkingChatMsgBaseCellData {
@objcMembers public class InfoCellData: ResultCellData {
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: str_sinkTitle.map{nameAdd(model: $0)}, encoding: .utf8)!)
    }
    ...
    override public var msgModel: TitleTagJsonModel {
        ...
        set {
            ...
            if newValue.msgInfo.content.isEmptyString {
                //: contentStr = "[未知消息]"
                contentStr = (String(str_textObjectContent) + str_signData.capitalized)
                //: }else {
            } else {
    ...
    public lazy var contentAttrString: NSMutableAttributedString = {
        ...
        let attr = overWill(msgInfoModel: self.msgModel.msgInfo)
        //: return attr
        return attr
        //: }()
    }()
    ...
    func overWill(msgInfoModel: UpModel) -> NSMutableAttributedString {
        ...
        guard dic[(str_indexMakeValue.replacingOccurrences(of: "share", with: "a"))] is [String: Any] else {
            //: return NSMutableAttributedString.init(string: "")
            return NSMutableAttributedString(string: "")
        }
        ...
        let textColor = (self.direction == .MsgDirectionIncoming) ? UIColor.segment() : .white
        //: let atColor = (self.direction == .MsgDirectionIncoming) ? UIColor(hex: "#0084FF")! : UIColor(hex: "#FFEE4B")!
        let atColor = (self.direction == .MsgDirectionIncoming) ? UIColor(hex: (String(str_bottomActionTitle)))! : UIColor(hex: (str_messageUserName.capitalized + str_imageInfoName.replacingOccurrences(of: "visual", with: "B")))!
        ...
        let regex = try! NSRegularExpression(pattern: (String(str_unknownName.suffix(5)) + "[\\S\\" + String(str_targetPushName.prefix(4)) + "</at>"), options: [])
        ...
        for (index, value) in rangeArr.enumerated() {
            ...
            if jumpInfo["jumpKey"] == "user" {
                ...
                uid = jumpInfo[String(bytes: str_hiddenValue.map{resultButton(color: $0)}, encoding: .utf8)!].intValue
            }
            //: if uid != -1 {
            if uid != -1 {
                ...
                attr.yy_setTextHighlight(range, color: atColor, backgroundColor: .clear, userInfo: [String(bytes: str_hiddenValue.map{resultButton(color: $0)}, encoding: .utf8)!: uid]) { [weak self] _, _, _, _ in
    ...
    func formatForText() -> NSMutableAttributedString {
        ...
        var color = UIColor(hex: (String(str_makeData)))!
        //: if self.translateStatus == .MsgTranslate_fail {
        if self.translateStatus == .MsgTranslate_fail {
            //: color = UIColor(hex: "#F95644")!
            color = UIColor(hex: (String(str_partyText)))!
        }
