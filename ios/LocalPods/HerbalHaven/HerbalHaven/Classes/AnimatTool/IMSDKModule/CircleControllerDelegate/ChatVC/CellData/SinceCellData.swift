...
fileprivate let str_textSharedValue:[UInt8] = [0xb,0xc,0xb,0x16,0x4a,0x1,0xd,0x6,0x7,0x10,0x58,0x4b,0x42,0xa,0x3,0x11,0x42,0xc,0xd,0x16,0x42,0x0,0x7,0x7,0xc,0x42,0xb,0xf,0x12,0xe,0x7,0xf,0x7,0xc,0x16,0x7,0x6]

private func giftView(list num: UInt8) -> UInt8 {
    return num ^ 98
}

/*: "View read receipts?" :*/
fileprivate let str_barMakerValue:String = "View reacell lab"
fileprivate let str_makeErrorButtonText:String = "d reimport make view video"
fileprivate let str_downPlayTitle:String = "ceipts?title model self insert"

/*: "text" :*/
fileprivate let str_tapContent:String = "tebot"

/*: "color" :*/
fileprivate let str_viewRefreshName:[UInt8] = [0x72,0x6f,0x6c,0x6f,0x63]

/*: "<br/>" :*/
fileprivate let str_equalBottomData:[Character] = ["<","b","r","/",">"]

/*: "\n" :*/
fileprivate let str_styleToContent:String = "\n"

/*: "<br />" :*/
fileprivate let str_touchText:[Character] = ["<"]
fileprivate let str_bagData:[Character] = ["b","r"," ","/",">"]

/*: "<xml>%@</xml>" :*/
fileprivate let str_transformData:[Character] = ["<","x","m","l",">","%","@","<","/","x","m","l"]
fileprivate let str_centerTitle:[Character] = [">"]

/*: "href" :*/
fileprivate let str_makeValue:[Character] = ["h","r","e","f"]

/*: "type" :*/
fileprivate let str_plusData:[UInt8] = [0x34,0x39,0x30,0x25]

private func hiddenStart(click num: UInt8) -> UInt8 {
    return num ^ 64
}

/*: "txt" :*/
fileprivate let str_equalContent:String = "tinterruptt"

/*: "padding" :*/
fileprivate let str_lastMakeName:String = "imageadding"

/*: "size" :*/
fileprivate let str_topText:[Character] = ["s","i","z","e"]

/*: "maxFont" :*/
fileprivate let str_systemShowTitle:String = "maxFontimage path"

/*: "link" :*/
fileprivate let str_titleCoinContent:[Character] = ["l","i","n","k"]

/*: "PingFangSC-Regular" :*/
fileprivate let str_normalDataName:String = "Pinghidden more please"
fileprivate let str_labelValue:String = "C-Reguimage force round data"
fileprivate let str_databaseTitle:String = "LAR"

/*: "content" :*/
fileprivate let str_cancelFormatIfTitle:[UInt8] = [0xf5,0xf9,0xf8,0xe2,0xf3,0xf8,0xe2]

private func viewEmpty(with num: UInt8) -> UInt8 {
    return num ^ 150
}
...
//: @objcMembers public class SinceCellData: TalkingChatMsgBaseCellData {
@objcMembers public class SinceCellData: ResultCellData {
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: str_textSharedValue.map{giftView(list: $0)}, encoding: .utf8)!)
    }
    ...
    override public func contentSize() -> CGSize {
        ...
        if self.msgModel.tips.content == (String(str_barMakerValue.prefix(8)) + String(str_makeErrorButtonText.prefix(4)) + String(str_downPlayTitle.prefix(7))).localized {
            //: return CGSize(width: ScreenWidth, height: 30)
            return CGSize(width: kLet_itemData, height: 30)
        }
        ...
        //: let rect = self.attributedString.boundingRect(with: CGSize.init(width: AreaReactiveCompatible.getScreenWidth()-50, height: CGFloat(MAXFLOAT)), options: [.usesLineFragmentOrigin, .usesFontLeading], context: nil )
        let rect = self.attributedString.boundingRect(with: CGSize(width: AreaReactiveCompatible.r() - 50, height: CGFloat(MAXFLOAT)), options: [.usesLineFragmentOrigin, .usesFontLeading], context: nil)
    ...
    public lazy var attributedString: NSMutableAttributedString = {
        ...
        var attr = formatPhotoNews(info: self.msgModel.tips)
        //: return attr
        return attr
        //: }()
    }()
...
//: extension SinceCellData {
extension SinceCellData {
    //: func formatMessageToStringM(info: TipsObjectJsonModel) -> NSMutableAttributedString {
    func formatPhotoNews(info: TipsObjectJsonModel) -> NSMutableAttributedString {
...
//        let dic = NSDictionary(jsonString: extra ?? "")
        ...
        return selectFirst(tips: content)
    }

    //: func designTextFream(tips: String) -> NSMutableAttributedString {
    func selectFirst(tips: String) -> NSMutableAttributedString {
        ...
        _ = equalInfo(msg: tips, info: attrbutedArray)
        //: self.yyLayout = designYYTextLayout(array: attrbutedArray, displayMaxWidth: AreaReactiveCompatible.getScreenWidth()-50, para: para)
        self.yyLayout = adhere(array: attrbutedArray, displayMaxWidth: AreaReactiveCompatible.r() - 50, para: para)
    ...
    func equalInfo(msg: String, info: NSMutableArray) -> String {
        ...
        let pure_msg = listBy(source: msg, attrbAM: contentAttrArray)
        //: for  textInfo in contentAttrArray {
        for textInfo in contentAttrArray {
            //: let textDic = textInfo as![String: Any]
            let textDic = textInfo as! [String: Any]
            //: let text: String = textDic["text"] as! String
            let text: String = textDic[(str_tapContent.replacingOccurrences(of: "bot", with: "xt"))] as! String

            //: var textColor: UIColor?
            var textColor: UIColor?

            //: var obj = textDic["color"]
            var obj = textDic[String(bytes: str_viewRefreshName.reversed(), encoding: .utf8)!]
            //: if obj is UIColor {
            if obj is UIColor {
                //: obj = obj as? UIColor
                obj = obj as? UIColor
            ...
            contentDict.removeObjects(forKeys: [String(bytes: str_viewRefreshName.reversed(), encoding: .utf8)!, (str_tapContent.replacingOccurrences(of: "bot", with: "xt"))])

            //: let attrDict = ctDictionaryForText(content: text, color: textColor)
            let attrDict = aggregationAdd(content: text, color: textColor)
    ...
    func listBy(source: String, attrbAM: NSMutableArray) -> String {
        ...
        do {
            ...
            let document = try ONOXMLDocument(string: String(format: (String(str_transformData) + String(str_centerTitle)), newSource), encoding: CFStringConvertEncodingToNSStringEncoding(CFStringBuiltInEncodings.UTF8.rawValue))
            //: let element = document.rootElement
            let element = document.rootElement
            //: parseXMLElement(element: element ?? ONOXMLElement(), arrM: attrbAM, attrib: ["color": UIColor.white])
            callByAttrib(element: element ?? ONOXMLElement(), arrM: attrbAM, attrib: [String(bytes: str_viewRefreshName.reversed(), encoding: .utf8)!: UIColor.white])
            ...
            attrbAM.add([(str_tapContent.replacingOccurrences(of: "bot", with: "xt")): newSource, String(bytes: str_viewRefreshName.reversed(), encoding: .utf8)!: UIColor.white])
            //: return newSource
            return newSource
        }
    }
    ...
    func callByAttrib(element: ONOXMLElement, arrM: NSMutableArray, attrib: NSDictionary) {
        ...
        if children.count > 0 {
            ...
            if element.tag == "u" { // 下滑线
                ...
                let url: String = element.attributes[(String(str_makeValue))] as! String
                //: if url.count > 0 {
                if url.count > 0 {
                    //: parentInfo["href"] = url
                    parentInfo[(String(str_makeValue))] = url
                }
            }
            //: for  subElement in children {
            for subElement in children {
                //: parseXMLElement(element: subElement as! ONOXMLElement, arrM: arrM, attrib: parentInfo)
                callByAttrib(element: subElement as! ONOXMLElement, arrM: arrM, attrib: parentInfo)
            }
            ...
            if text != nil {
                ...
                //: info["text"] = text
                info[(str_tapContent.replacingOccurrences(of: "bot", with: "xt"))] = text
                //: arrM.add(info)
                arrM.add(info)
            }
        }
    ...
    func adhere(array: NSArray, displayMaxWidth: CGFloat, para: NSParagraphStyle?) -> YYTextLayout {
        //: let attrString = designAttribStr(array: array, paragraphStyle: para)
        let attrString = barIn(array: array, paragraphStyle: para)
    ...
    func barIn(array: NSArray, paragraphStyle: NSParagraphStyle?) -> NSAttributedString {
        ...
        array.enumerateObjects { obj, _, _ in
            ...
            let type = newObj[String(bytes: str_plusData.map{hiddenStart(click: $0)}, encoding: .utf8)!] as? String
            //: var tmp = NSAttributedString.init()
            var tmp = NSAttributedString()
            //: if type  == "txt" {
            if type == (str_equalContent.replacingOccurrences(of: "interrupt", with: "x")) {
                //: tmp = designAttribStrByTextDic(dict: newObj as NSDictionary)
                tmp = plot(dict: newObj as NSDictionary)
                //: } else if type == "padding" {
            } else if type == (str_lastMakeName.replacingOccurrences(of: "image", with: "p")) {
                //: let imageView = UIImageView.init()
                let imageView = UIImageView()
                //: imageView.size = (newObj["size"] as! NSValue).cgSizeValue
                imageView.size = (newObj[(String(str_topText))] as! NSValue).cgSizeValue
                ...
                tmp = NSMutableAttributedString.yy_attachmentString(withContent: imageView, contentMode: .center, attachmentSize: imageView.size, alignTo: newObj[(String(str_systemShowTitle.prefix(7)))] as! UIFont, alignment: .center)
            }
            ...
            if underline {
                ...
                if len > 0 {
                    ...
                    _ = YYTextDecoration(style: .single, width: 1.5, color: newObj[String(bytes: str_viewRefreshName.reversed(), encoding: .utf8)!] as? UIColor)
    ...
    func currentContent(dict: NSDictionary) -> NSAttributedString {
        //: let imageView = UIImageView.init()
        let imageView = UIImageView()
        //: imageView.size = (dict["size"]as! NSValue).cgSizeValue
        imageView.size = (dict[(String(str_topText))] as! NSValue).cgSizeValue
        ...
        let attachText = NSMutableAttributedString.yy_attachmentString(withContent: imageView, contentMode: .center, attachmentSize: imageView.size, alignTo: dict[(String(str_systemShowTitle.prefix(7)))] as! UIFont, alignment: .center)
        //: return attachText
        return attachText
    }

    //: func designAttribStrByTextDic(dict: NSDictionary) ->NSMutableAttributedString {
    func plot(dict: NSDictionary) -> NSMutableAttributedString {
        //: let type = dict["type"] as! String
        let type = dict[String(bytes: str_plusData.map{hiddenStart(click: $0)}, encoding: .utf8)!] as! String
        //: if type != "link" && type != "txt" {
        if type != (String(str_titleCoinContent)) && type != (str_equalContent.replacingOccurrences(of: "interrupt", with: "x")) {
            //: return NSMutableAttributedString.init()
            return NSMutableAttributedString()
        }
        //: let attributes = attributesWithConfig()
        let attributes = beforeWith()
        // set color
        //: let color = dict["color"] as? UIColor
        let color = dict[String(bytes: str_viewRefreshName.reversed(), encoding: .utf8)!] as? UIColor
        //: if color != nil {
        if color != nil {
        ...
        let fontSize = (dict[(String(str_topText))] as? NSNumber)?.floatValue
        //: if (fontSize ?? 0 > 0) {
        if fontSize ?? 0 > 0 {
            //: let fontRef = CTFontCreateWithName("PingFangSC-Regular" as CFString, CGFloat(fontSize!), nil)
            let fontRef = CTFontCreateWithName((String(str_normalDataName.prefix(4)) + "FangS" + String(str_labelValue.prefix(6)) + str_databaseTitle.lowercased()) as CFString, CGFloat(fontSize!), nil)
            //: attributes[kCTFontAttributeName] = fontRef
            attributes[kCTFontAttributeName] = fontRef
        }
        //: var content = (dict["content"] != nil) ? dict["content"] : ""
        var content = (dict[String(bytes: str_cancelFormatIfTitle.map{viewEmpty(with: $0)}, encoding: .utf8)!] != nil) ? dict[String(bytes: str_cancelFormatIfTitle.map{viewEmpty(with: $0)}, encoding: .utf8)!] : ""
    ...
    func beforeWith() -> NSMutableDictionary {
        //: let font = UIFont.moveDismiss(fontSize: 15)
        let font = UIFont.moveDismiss(fontSize: 15)
    ...
    func aggregationAdd(content: String, color: UIColor?) -> NSDictionary {
        //: if content.count <= 0 {
        if content.count <= 0 {
        ...
        if color == nil {
            ...
            return fileText(content: content)
        }
        //: return ["type": "txt",
        return [String(bytes: str_plusData.map{hiddenStart(click: $0)}, encoding: .utf8)!: (str_equalContent.replacingOccurrences(of: "interrupt", with: "x")),
                //: "content": content,
                String(bytes: str_cancelFormatIfTitle.map{viewEmpty(with: $0)}, encoding: .utf8)!: content,
                //: "color": color ?? .black]
                String(bytes: str_viewRefreshName.reversed(), encoding: .utf8)!: color ?? .black]
    }

    //: func ctDictionaryForText(content: String) ->NSDictionary {
    func fileText(content: String) -> NSDictionary {
        //: if content.count <= 0 {
        if content.count <= 0 {
            //: return NSDictionary.init()
            return NSDictionary()
