...
- (void)setupViews {
    ...
    [[NSNotificationCenter defaultCenter] addObserver:self
                                             selector:@selector(menuWillHide:)
                                               ...
                                               object:nil];
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(onNewMessage:) name:[ClickThen textCell] object:nil];
...
- (void)loadMessage {
    ...
    if (self.currentChatType == TalkingIMChatType_group ||
        ...
        [[V2TIMManager sharedInstance] getGroupHistoryMessageList:self.targetID count:msgCount lastMsg:self.msgForGet succ:^(NSArray<V2TIMMessage *> *msgs) {
            ...
            if ([SharedConversationListener shared].isConnection) { //推送点进来会报错
                NSString *str = [@"Net Error, Try again later" localized];
                [self talkTaskWithShowMsg:[str localized]];
            }
        ...
        [[V2TIMManager sharedInstance] getC2CHistoryMessageList:self.targetID count:msgCount lastMsg:self.msgForGet succ:^(NSArray<V2TIMMessage *> *msgs) {
            ...
            if ([SharedConversationListener shared].isConnection) {                
                if (RecordAppManager.share.appStatus == 0) {
                    NSString *str = [@"Net Error, Try again later" localized];
                    [self talkTaskWithShowMsg:[str localized]];
                }
            }
...
- (void)getMessages:(NSArray *)msgs msgCount:(int)msgCount {
    ...
    for (NSInteger k = array.count - 1; k >= 0; --k) {
        ...
        if ([userId isEqualToString:RecordAppManager.share.loginUserMode.userID]) {
...
- (void)onNewMessage:(NSNotification *)notification
{
    ...
    if ([msg.sender isEqualToString:RecordAppManager.share.loginUserMode.userID]) {
        //过滤本人发送的消息
        return;
    }
    ...
    if (uiMsgs.count > 0) {
        ...
        if ((self.tableView.contentSize.height - self.tableView.contentOffset.y) < [AreaReactiveCompatible getUser] * 1.5) {
...
- (CGFloat)tableView:(UITableView *)tableView heightForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    //    缓存存在问题待处理
    //    if (height > 0 && ![data isKindOfClass:[AbFitInCellData class]] && ![data isKindOfClass:[SinceCellData class]]) {
    //        return height;
    //    }
...
- (void)sendCustomMessage:(TUIMessageCellData *)msg {
    ...
    NSInteger sender = RecordAppManager.share.loginUserMode.userID.integerValue;
    NSString *msgID;
    if (isGroup) {
...
- (void)modifyCustomMessageDataWithExtral:(NSDictionary *)extral cellData:(TUIMessageCellData *)msg {
    ...
    // 更新内存模型
    msg.msgModel = [TitleTagJsonModel modelWithDic:newExtraDict];
}
...
///   - msg: 图片消息
///   - model: 私密照片模型
- (void)changeIntimatePhotoMsg:(TUIMessageCellData *)msg photoModel:(HiddenJsonModel *)model {
    dispatch_async(dispatch_get_main_queue(), ^{
        if (model == nil) { return; }
        [HiddenJsonModel render:model quote:msg.msgID];
...
///   - msg: 视频消息
///   - model: 私密视频模型
- (void)changeIntimateVideoMsg:(TUIMessageCellData *)msg videoModel:(BriquetModel *)model {
    dispatch_async(dispatch_get_main_queue(), ^{
        if (model == nil) { return; }
        [BriquetModel head:model unwishedScale:msg.msgID];
...
- (void)changeIntimateGiftMsg:(TUIMessageCellData *)msg {
    dispatch_async(dispatch_get_main_queue(), ^{
        [TitleJsonModel frame:msg.msgID];
...
- (TalkingSystemMsgCellData *)transSystemMsgFromDate:(NSDate *)date
{
    if(_msgForDate == nil || fabs([date timeIntervalSinceDate:_msgForDate.timestamp]) > MAX_MESSAGE_SEP_DLAY) {
        ...
        system.contentStr = [NSDate goWithShowDetail:YES date:date];
...
- (void)onLongPressMessageWithCell:(TUIMessageCell *)cell{
    ...
    BOOL hasJumps = NO;
    if ([data isKindOfClass:[SerrationCellData class]]) {
        SerrationCellData *textCellData = (SerrationCellData *)data;
    ...
    // 只有文字才有复制功能
    if (([data isKindOfClass:[InfoCellData class]] ||
         [data isKindOfClass:[SerrationCellData class]]) && hasJumps == NO) {
        // 过滤掉音视频通话文本消息
...
- (NSArray *)requiredResponseWithLongGestureRecognizer {
    return @[[InfoCellData class],
             [SerrationCellData class],
             [NameBalloonThen class],
             [UserCellData class],
             [MarkMakeCellData class],
             [SoundCellData class],
             [HorizonCellData class],
             [PartCellData class]];
}
...
- (void)onCopyMsg:(id)sender {
    ...
    if ([_menuUIMsg isKindOfClass:[InfoCellData class]] ||
        [_menuUIMsg isKindOfClass:[SerrationCellData class]]) {
        SerrationCellData *txtMsg = (SerrationCellData *)_menuUIMsg;
...
- (BOOL)canReplyResponseWithLongGestureRecognizer:(TUIMessageCellData *)cellData {
    ...
    if ([cellData isKindOfClass:[SoundCellData class]] || [cellData isKindOfClass:[NameBalloonThen class]] || [cellData isKindOfClass:[PartCellData class]]) {
        return YES;
    }
...
- (void)translateMsgWithCellData:(TUIMessageCellData *)cellData {
    ...
    if (RecordAppManager.share.appUserConfigMode.enableTranslate == NO) { // 非会员，无法使用翻译
        [[EnvelopeSinceAdministratorThen share] subscribe];
        return;
    }
    
    if ([cellData isKindOfClass:[SerrationCellData class]]) {
        SerrationCellData *txtMsg = (SerrationCellData *)cellData;
        [self translateMessageToReq:txtMsg andType:0];
        
    } else if ([cellData isKindOfClass:[InfoCellData class]]) {
        InfoCellData *txtMsg = (InfoCellData *)cellData;
...
- (void)translateMessageToReq:(TUIMessageCellData *)txtMsg andType:(int)type {
    ...
    // 调用翻译接口
    [LiveRowRequestTool variorumEditionWithMsgId:txtMsg.msgModel.msgInfo.msgId type:type completion:^(BOOL succeed, NSDictionary *dict, NSInteger errorCode) {
        if (!succeed) {
            if (errorCode == 403) { // 非会员，无法使用翻译
                RecordAppManager.share.appUserConfigMode.enableTranslate = NO;
                [[EnvelopeSinceAdministratorThen share] subscribe];
            }
