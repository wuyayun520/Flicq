...
/*: "id" :*/
fileprivate let str_beforeTitle:[Character] = ["i","d"]

/*: "num" :*/
fileprivate let str_theMainTitle:[Character] = ["n","u","m"]
...
@objc public enum NameEquatable: Int {
    //: case normal  = 1
    case normal = 1 // 普通礼物
    //: case myStery = 2
    case myStery = 2 // 盲盒礼物
...
struct PartAnimatModel: HandyJSON {
    //: init() {
    init() {}
    ...
    mutating func complete(giftNum: Int) {
        //: self.num = giftNum
        self.num = giftNum
    }
}
...
struct TableDHoteAnimatModel: HandyJSON {
    //: init() {
    init() {}
    ...
    mutating func mapping(mapper: HelpingMapper) {
        ...
        mapper <<<
            //: self.giftNum <-- "num"
            self.giftNum <-- (String(str_theMainTitle))
    }
}
