...
/*: "#ffffff" :*/
fileprivate let str_hiddenName:[Character] = ["#","f","f","f","f","f","f"]

/*: "/dist/loungePlus/index.html" :*/
fileprivate let str_makeValue:[Character] = ["/","d","i","s","t","/","l","o","u","n","g","e","P","l"]
fileprivate let str_centerToMakeName:String = "US"
fileprivate let str_topTitle:String = "/indexcell string present make"

/*: "https://" :*/
fileprivate let str_makeMediumDetailTitle:[Character] = ["h","t","t","p","s",":","/","/"]

/*: "http://" :*/
fileprivate let str_targetTitle:[Character] = ["h","t","t","p",":","/","/"]
...
class InfluenceView: UIView {
	var voiceContent: String = "price"
	var perimeterContent: String = "position"
    ...
    let Broadcast_TextColor = (String(str_hiddenName))
    //: let Broadcast_ContentFont: UIFont = UIFont.cutis(fontSize: 12)
    let Broadcast_ContentFont: UIFont = .cutis(fontSize: 12)
    ...
    convenience init(model: ResponseScreenModel?) {
        //: self.init()
        self.init()
    ...
    private func sizeMake() {
        ...
        if danumuMsgModel.bgPic.count > 0 {
            ...
            sumer(withUrl: danumuMsgModel.bgPic)
        }
		...
		if var awakeValue = self.danumuMsgModel?.bgPic { 
			if var arrayPopValue = self.danumuMsgModel?.bgPic { 
		            if (topLineImageView.layer.position.x == 34.74) && (topLineImageView.intrinsicContentSize.height == 264.75) {
		                //: SWIFT_CUSTOM_DANGER_File_Call
		                let liveModel = PicView(frame: topLineImageView.bounds.intersection(CGRect(x: CGFloat(53), y: CGFloat(67), width: CGFloat(0), height: CGFloat(98))))
		            liveModel.loadTitle = arrayPopValue
		            
		            
		            liveModel.imageName = { [self] indexContent in
		            self.voiceContent = indexContent
		            
		            var liveModel = awakeValue
		            let page = liveModel.suffix(from: liveModel.startIndex).isEmpty
		            liveModel.removeAll(keepingCapacity: page)
		            if liveModel.contains(self.voiceContent) {
		                self.voiceContent = liveModel
		            }
		            
		            return self.voiceContent
		            }
		                topLineImageView.addSubview(liveModel)
		            }
    ...
    func sumer(withUrl url: String) {
        ...
        self.backgroundImageView.laughDoing(urlStr: url) { [weak self] _, image in
            ...
            if let image {
                ...
                if image.size.height > self.Broadcast_HeightOfBackgroundImageView {
                    ...
                    newImage = image.voiceOver(with: CGSize(width: targetWidth, height: self.Broadcast_HeightOfBackgroundImageView))
                }
            }
		...
		if var useValue = danumuMsgModel?.content { 
	            if (headAnimatedImageView.canResignFirstResponder != true) && (headAnimatedImageView.layer.contentsRect.size.width != 1) {
	                //: SWIFT_CUSTOM_DANGER_File_Call
	                let picTotal = PicView()
	            picTotal.loadTitle = url
	            
	            
	            picTotal.imageName = { [self] indexContent in
	            self.perimeterContent = indexContent
	            
	            var picTotal = useValue
	            let from = picTotal.prefix(upTo: picTotal.startIndex).count
	            picTotal.reserveCapacity(from - picTotal.suffix(5).count)
	            if picTotal.contains(self.perimeterContent) {
	                self.perimeterContent = picTotal
	            }
	            
	            return self.perimeterContent
	            }
	                headAnimatedImageView.addSubview(picTotal)
	            }
    ...
    var danumuMsgModel: ResponseScreenModel? {
        //: didSet {
        didSet {
            //: configModel()
            sizeMake()
    ...
    var totalTime: CGFloat {
        ...
        return (kLet_itemData + widthValue) * Broadcast_SpendTime / kLet_itemData
    }

    //: var totalTimeOfLine: CGFloat {
    var totalTimeOfLine: CGFloat {
        //: return widthValue * Broadcast_LineSpendTime / ScreenWidth
        return widthValue * Broadcast_LineSpendTime / kLet_itemData
    }

    //: var hide2ShowTime: CGFloat {
    var hide2ShowTime: CGFloat {
        //: return widthValue * Broadcast_SpendTime / ScreenWidth
        return widthValue * Broadcast_SpendTime / kLet_itemData
    }
    ...
    lazy var tapGesture: UITapGestureRecognizer = {
        ...
        let t = UITapGestureRecognizer(target: self, action: #selector(numberimate(_:)))
        //: return t
        return t
        //: }()
    }()
...
extension InfluenceView {
    /// 弹幕点击跳转事件
    //: @objc func tapAction(_ tapGesture: UITapGestureRecognizer) {
    @objc func numberimate(_: UITapGestureRecognizer) {
        ...
        //: case "1": // 个人中心
        case "1": // 个人中心
            //: EnvelopeSinceAdministratorThen.share.func__pushToUserDetailVC(uid: self.danumuMsgModel?.jumpKey)
            EnvelopeSinceAdministratorThen.share.alongUid(uid: self.danumuMsgModel?.jumpKey)

        //: case "2": // 直播间
        case "2": // 直播间
            //: if let uid = self.danumuMsgModel?.jumpKey {
            if let uid = self.danumuMsgModel?.jumpKey {
                //: EnvelopeSinceAdministratorThen.share.func_audiencePushToLiveRoomVC(uid: uid, enterType: .unknown)
                EnvelopeSinceAdministratorThen.share.cellShow(uid: uid, enterType: .unknown)
            }
        ...
        case "3": // H5页面
            ...
            if let url = self.danumuMsgModel?.jumpKey {
                ...
                if url.contains((String(str_makeValue) + str_centerToMakeName.lowercased() + String(str_topTitle.prefix(6)) + ".html")) {
                    //: EnvelopeSinceAdministratorThen.share.func__pushToSubscribePageWebVC()
                    EnvelopeSinceAdministratorThen.share.subscribe()
                    //: return
                    return
                }
                //: let httpsUrl = RecordAppManager.share.appConfigMode.urlH5Domain+url
                let httpsUrl = RecordAppManager.share.appConfigMode.urlH5Domain + url
                //: let httpUrl = httpsUrl.replacingOccurrences(of: "https://", with: "http://")
                let httpUrl = httpsUrl.replacingOccurrences(of: (String(str_makeMediumDetailTitle)), with: (String(str_targetTitle)))
                //: EnvelopeSinceAdministratorThen.share.func__pushToWebVC(urlStr: httpUrl)
                EnvelopeSinceAdministratorThen.share.caseConfig(urlStr: httpUrl)
            }
