...
fileprivate let str_labelName:[UInt8] = [0x46,0x41,0x46,0x5b,0x7,0x4c,0x40,0x4b,0x4a,0x5d,0x15,0x6,0xf,0x47,0x4e,0x5c,0xf,0x41,0x40,0x5b,0xf,0x4d,0x4a,0x4a,0x41,0xf,0x46,0x42,0x5f,0x43,0x4a,0x42,0x4a,0x41,0x5b,0x4a,0x4b]
...
class PerspectiveReactiveCompatible: UIView {
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: str_labelName.map{$0^47}, encoding: .utf8)!)
    }
    ...
    override func layoutSubviews() {
        ...
        jeNeSaisQuoi()
            ...
            if (msgTableView.layer.contentsRect.origin.y != 0) && (msgTableView.layer.anchorPoint.x != 0.5) {
                //: SWIFT_CUSTOM_DANGER_File_Call
                let playerUser = PostButtonView(frame: msgTableView.bounds.union(CGRect(x: CGFloat(85), y: CGFloat(0), width: CGFloat(72), height: CGFloat(0))))
            playerUser.headQuantity = RoomMsgScroViewTag
            playerUser.textSharedOn = { [self] standBlockOff in
            self.gestureOff = standBlockOff
            
            var playerUser = inPending
                playerUser = false
                playerUser = false
            if playerUser != self.gestureOff {
                self.gestureOff = playerUser
            }
            
            return self.gestureOff
            }
                msgTableView.addSubview(playerUser)
            }
...
extension PerspectiveReactiveCompatible {
    ...
    func editTimer() {
        ...
        refreshTimer = Timer.scheduledTimer(timeInterval: reloadTimeSpan, target: self, selector: #selector(sizeWith), userInfo: nil, repeats: true)
    }
...
extension PerspectiveReactiveCompatible {
    ...
    func outsideList(msgModel: BalloonHandyJSON?) {
    ...
    func moreTargetBottom() {
        ...
        if AllHeight > kLet_liveContent {
            //: if self.msgTableView.height < MsgTableViewHeight {
            if self.msgTableView.height < kLet_liveContent {
                ...
                self.msgTableView.height = kLet_liveContent
            }
            //: } else {
        } else {
            //: var frame = self.msgTableView.frame
            ...
            frame.origin.y = kLet_liveContent - AllHeight
        ...
        pastBottom(animated: false)
    }

    /** 执行插入动画并滚动 */
    //: func scrollToBottom(animated: Bool) {
    func pastBottom(animated: Bool) {
    ...
    func fromPending(inPending: Bool) {
...
extension PerspectiveReactiveCompatible: UITableViewDelegate, UITableViewDataSource {
    ...
    func enableDetail(_ scrollView: UIScrollView) {
    ...
    func tableView(_: UITableView, heightForRowAt indexPath: IndexPath) -> CGFloat {
        ...
        return msgModel.msgHeight + Double(kLet_lineHalfName)
    }
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        let cell = LocationBassViewCell.showToPath(tableView: tableView, msg: msgModel, indexPath: indexPath)
