...
/*: "99+" :*/
fileprivate let str_partySizeGuardData:String = "showshow+"

/*: "init(coder:) has not been implemented" :*/
fileprivate let str_managerRoomValue:[UInt8] = [0x2b,0x2c,0x2b,0x36,0x6a,0x21,0x2d,0x26,0x27,0x30,0x78,0x6b,0x62,0x2a,0x23,0x31,0x62,0x2c,0x2d,0x36,0x62,0x20,0x27,0x27,0x2c,0x62,0x2b,0x2f,0x32,0x2e,0x27,0x2f,0x27,0x2c,0x36,0x27,0x26]

private func dataConverter(move num: UInt8) -> UInt8 {
    return num ^ 66
}

/*: "#2DF2FF" :*/
fileprivate let str_blockData:[Character] = ["#","2","D","F","2","F","F"]

/*: "Live" :*/
fileprivate let str_cellEqualName:[Character] = ["L","i","v","e"]

/*: "img_home_shadow_icon" :*/
fileprivate let str_failData:[Character] = ["i","m","g","_","h","o","m","e","_","s"]
fileprivate let str_modelPlayerData:[Character] = ["h","a"]
fileprivate let str_managerData:[Character] = ["d","o","w","_","i","c","o","n"]

/*: "get json error" :*/
fileprivate let str_laboratoryIndexName:[Character] = ["g","e","t"," "]
fileprivate let str_styleObjectData:String = "text as photo selfjson e"

/*: "#FF2348" :*/
fileprivate let str_observerMakeName:String = "#FF2348inside hidden view mode"

/*: "#35C759" :*/
fileprivate let str_acceptViewCenterValue:String = "#35C759list start"

/*: "mfBean" :*/
fileprivate let str_sourceWillValueContent:[Character] = ["m","f","B","e","a","n"]

/*: "%.1f" :*/
fileprivate let str_addHiddenViewTitle:[Character] = ["%",".","1","f"]

/*: "points\n≈ $" :*/
fileprivate let str_laboratoryScreenValue:String = "pgiftn"

/*:   :*/
fileprivate let str_makeData:String = "play"
...
public var kLet_sizeSeatValue: Int {
    //: return RecordAppManager.share.appConfigMode.countdown1 + RecordAppManager.share.appConfigMode.countdown2
    return RecordAppManager.share.appConfigMode.countdown1 + RecordAppManager.share.appConfigMode.countdown2
}

//: class TalkingLiveMiniView: TalkingBaseMiniView {
class ResultReactiveCompatible: LoopLightMiniView {
    // 未检测出人脸时长（间隔1秒）
    ...
    var noFaceTime: Int = 0 {
        ...
        didSet {
            ...
            if noFaceTime >= RecordAppManager.share.appConfigMode.countdown1 { // 展示倒计时
                ...
                svgaNumLab.text = "\(kLet_sizeSeatValue - noFaceTime)"
                //: } else {
            } else {
                //: refreshMiniView()
                viewCamera()
    ...
    override init(frame: CGRect) {
        ...
        //: if Double(RecordAppManager.share.loginUserMode.mf_bean) > 0 {
        if Double(RecordAppManager.share.loginUserMode.mf_bean) > 0 {
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: str_managerRoomValue.map{dataConverter(move: $0)}, encoding: .utf8)!)
    }
    ...
    private lazy var liveStatusImgV: EveningShirtFixingTalkingButton = {
        ...
        let image = UIImage.makeBy(color: UIColor(hex: (String(str_blockData)))!, size: CGSize(width: 8, height: 8))
        //: img.setImage(image.data(), for: .normal)
        img.setImage(image.data(), for: .normal)
        //: img.setTitle("Live".localized, for: .normal)
        img.setTitle((String(str_cellEqualName)).localized, for: .normal)
        //: img.isHidden = true
        img.isHidden = true
        //: self.addSubview(img)
        self.addSubview(img)
    ...
    private lazy var shadowImg: UIImageView = {
        ...
        img.image = UIImage.markTag(name: (String(str_failData) + String(str_modelPlayerData) + String(str_managerData)))
        //: img.layer.cornerRadius = 8
        img.layer.cornerRadius = 8
        //: img.layer.masksToBounds = true
        img.layer.masksToBounds = true
    ...
    private lazy var svgaPlayer: SVGAPlayer = {
        ...
        let url = EffectTool.default.confirm(type: .Live_countdown_bg)
        //: do {
        do {
            ...
            printLog(message: (String(str_laboratoryIndexName) + String(str_styleObjectData.suffix(6)) + "rror"))
        }
        //: return player
        return player
        //: }()
    ...
    private lazy var svgaNumLab: UILabel = {
        ...
        lab.textColor = UIColor(hex: (String(str_observerMakeName.prefix(7))))
        //: self.addSubview(lab)
        self.addSubview(lab)
        //: return lab
        return lab
    ...
    private lazy var msgCountLab: UILabel = {
        ...
        lab.backgroundColor = UIColor(hex: (String(str_acceptViewCenterValue.prefix(7))))
...
extension ResultReactiveCompatible {
    ...
    static func spectaclesCellForefrontView() -> ResultReactiveCompatible {
        ...
        let position = ManagingDirectorReactiveCompatible.shared.liveMiniPosition
    ...
    func viewCamera() {
        ...
        liveStatusImgV.isHidden = !CellPushListener.inserter().isLive
...
extension ResultReactiveCompatible {
    ...
    private func lifeStyle() {
        ...
        NotificationCenter.default.addObserver(self,
                                               ...
                                               selector: #selector(onMake(notification:)),
                                               //: name: LIVE_USER_POINT_CHANGE,
                                               name: kLet_requestSucceedValue,
                                               //: object: nil)
                                               object: nil)
    }

    //: @objc private func pointchange(notification: NSNotification) {
    @objc private func onMake(notification: NSNotification) {
        //: let userinfo = notification.userInfo as! [String: AnyObject]
        let userinfo = notification.userInfo as! [String: AnyObject]
        //: guard let mfBean = userinfo["mfBean"] as? Double else { return }
        guard let mfBean = userinfo[(String(str_sourceWillValueContent))] as? Double else { return }
        //: setPointText(mfBean: mfBean)
        month(mfBean: mfBean)
    }

    //: func setPointText(mfBean: Double) {
    func month(mfBean: Double) {
        //: let nowBean = RecordAppManager.share.loginUserMode.mf_bean + mfBean
        let nowBean = RecordAppManager.share.loginUserMode.mf_bean + mfBean
        //: RecordAppManager.share.loginUserMode.mf_bean = nowBean
        RecordAppManager.share.loginUserMode.mf_bean = nowBean
        ...
        mfBeanLB.text = "\(Int(nowBean))\((str_laboratoryScreenValue.replacingOccurrences(of: "gift", with: "oi") + "ts\n\u{2248} $").localized) \(divisor)"
