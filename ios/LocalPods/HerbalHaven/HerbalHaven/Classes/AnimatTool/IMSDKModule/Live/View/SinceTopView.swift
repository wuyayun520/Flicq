...
/*: s" :*/
fileprivate let str_norName:[Character] = ["s"]

/*: "init(coder:) has not been implemented" :*/
fileprivate let str_loadTitle:[UInt8] = [0x5f,0x64,0x5f,0x6a,0x1e,0x59,0x65,0x5a,0x5b,0x68,0x30,0x1f,0x16,0x5e,0x57,0x69,0x16,0x64,0x65,0x6a,0x16,0x58,0x5b,0x5b,0x64,0x16,0x5f,0x63,0x66,0x62,0x5b,0x63,0x5b,0x64,0x6a,0x5b,0x5a]

fileprivate func giftLast(actual num: UInt8) -> UInt8 {
...
/*: "bth_live_follow_pre" :*/
fileprivate let str_voiceValue:String = "point letbth_"
fileprivate let str_betweenBottomName:String = "fmanagerl"
fileprivate let str_colorPhotoName:String = "video block inow_pre"

/*: "btn_live_mini_nor" :*/
fileprivate let str_barButtonData:[Character] = ["b","t","n","_","l","i","v","e","_","m","i","n","i","_","n","o","r"]

/*: "#2DF2FF" :*/
fileprivate let str_frameNextName:[Character] = ["#","2","D","F","2","F","F"]

/*: "Live" :*/
fileprivate let str_leadingMainName:String = "Livemini error time"

/*: "#FF506D" :*/
fileprivate let str_resultViewTitle:String = "manager height gift in#FF506D"

/*: "  :*/
fileprivate let str_deadlineSendTitle:String = " "

/*: "removeAttentionUid" :*/
fileprivate let str_arrayBroadText:String = "log like colorremoveA"
fileprivate let str_playBackText:String = "modelmodelen"

/*: "attentionUid" :*/
fileprivate let str_toAreaData:[Character] = ["a","t","t"]
fileprivate let str_viewMainData:String = "ENTIO"

/*: "source" :*/
fileprivate let str_picValue:[Character] = ["s","o","u","r","c","e"]

/*: "num" :*/
fileprivate let str_playerText:[Character] = ["n","u","m"]

/*: "UID: :*/
fileprivate let str_imageCurrentTitle:String = "UID:view stack self"

/*: "btn_live_closed_nor" :*/
fileprivate let str_topTitle:String = "dismiss path view up selfbtn_l"
fileprivate let str_toolPicName:String = "size image view withive_c"
...
class SinceTopView: UIView {
    //: weak var weakVC: TalkingBaseViewController?
    weak var weakVC: ToControllerPositionBaseRecognizerDelegate?
    // 右侧按钮点击Block
    ...
    var noFaceTime: Int = 0 {
        //: didSet {
        didSet {
            //: if noFaceTime >= RecordAppManager.share.appConfigMode.countdown1 {
            if noFaceTime >= RecordAppManager.share.appConfigMode.countdown1 { // 展示倒计时
                //: self.liveStatusImgV.isHidden = true
                self.liveStatusImgV.isHidden = true
                //: self.countdownView.isHidden = false
                self.countdownView.isHidden = false
                //: self.countdownLab.text = "\(MAX_NOFACETIME-noFaceTime)s"
                self.countdownLab.text = "\(kLet_sizeSeatValue - noFaceTime)s"
                // svga
                //: self.svgaView.isHidden = false
                self.svgaView.isHidden = false
                //: self.svgaView.refreshView(countdown: "\(MAX_NOFACETIME-noFaceTime)")
                self.svgaView.workingPapers(countdown: "\(kLet_sizeSeatValue - noFaceTime)")
                //: } else {
            } else {
    ...
    private var liveModel = ErrorRoomModel()
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: str_loadTitle.map{giftLast(actual: $0)}, encoding: .utf8)!)
    }
    ...
    private lazy var iconBgBtn: UIButton = {
        ...
        v.addTarget(self, action: #selector(listen), for: .touchUpInside)
        //: return v
        return v
        //: }()
    }()
    ...
    private lazy var followBtn: UIButton = {
        ...
        btn.setImage(UIImage.markTag(name: (String(str_voiceValue.suffix(4)) + "live_" + str_betweenBottomName.replacingOccurrences(of: "manager", with: "ol") + String(str_colorPhotoName.suffix(6)))), for: .normal)
        //: btn.addTarget(self, action: #selector(followButtonClick), for: .touchUpInside)
        btn.addTarget(self, action: #selector(notWindowClick), for: .touchUpInside)
        //: return btn
        return btn
        //: }()
    }()
    ...
    private lazy var miniBtn: UIButton = {
        ...
        btn.setImage(UIImage.markTag(name: (String(str_barButtonData))), for: .normal)
        //: btn.addTarget(self, action: #selector(miniButtonClick), for: .touchUpInside)
        btn.addTarget(self, action: #selector(objectStatus), for: .touchUpInside)
    ...
    private lazy var liveStatusImgV: EveningShirtFixingTalkingButton = {
        ...
        let image = UIImage.makeBy(color: UIColor(hex: (String(str_frameNextName)))!, size: CGSize(width: 8, height: 8))
        //: img.setImage(image.data(), for: .normal)
        img.setImage(image.data(), for: .normal)
        //: img.setTitle("Live".localized, for: .normal)
        img.setTitle((String(str_leadingMainName.prefix(4))).localized, for: .normal)
        //: img.isHidden = true
        img.isHidden = true
        //: return img
        return img
    ...
    private lazy var countdownView: UIView = {
        ...
        icon.backgroundColor = UIColor(hex: (String(str_resultViewTitle.suffix(7))))
    ...
    private lazy var countdownLab: UILabel = {
        ...
        //: lab.textColor = .white
        lab.textColor = .white
        //: lab.text = " \(RecordAppManager.share.appConfigMode.countdown2)s"
        lab.text = " \(RecordAppManager.share.appConfigMode.countdown2)s"
    ...
    lazy var svgaView: EqualShowReactiveCompatible = {
        //: let v = TalkingLiveCountdownView()
        let v = EqualShowReactiveCompatible()
        //: v.isHidden = true
        v.isHidden = true
        //: return v
        return v
    ...
    private lazy var onlineNumBtn: UIButton = {
        ...
        btn.addTarget(self, action: #selector(plunge), for: .touchUpInside)
...
extension SinceTopView {
    ...
    @objc func part(uid: Int, isAttention: Bool, completion: ((_ succeed: Bool) -> Void)? = nil) {
        ...
        if isAttention {
            ...
            dict[(String(str_arrayBroadText.suffix(7)) + str_playBackText.replacingOccurrences(of: "model", with: "t") + "tionUid")] = uid
            //: } else {
        } else {
            //: dict["attentionUid"] = uid
            dict[(String(str_toAreaData) + str_viewMainData.lowercased() + "nUid")] = uid
            //: dict["source"] = "1"
            dict[(String(str_picValue))] = "1"
        }
        //: LiveRowRequestTool.req_atationTool(isAttention: !isAttention, params: dict) { succeed, result, errorModel in
...
extension SinceTopView {
    ...
    @objc private func plunge() {
        ...
        self.onlineListView.equalRed(chatGroupId: liveModel.chatGroupId)
    }
    ...
    @objc private func margin() {
        ...
        ColumnThen().recordModel(chatGroupId: liveModel.chatGroupId, completion: { succeed, result, _ in
            //: guard succeed else { return }
            guard succeed else { return }
            //: let json = JSON(result ?? [String: Any]())
            let json = JSON(result ?? [String: Any]())
            //: let num = json["num"].string
            let num = json[(String(str_playerText))].string
    ...
    private func underway() {
        ...
        onlineNunTimer = Timer.scheduledTimer(timeInterval: 10, target: self, selector: #selector(margin), userInfo: nil, repeats: true)
    }

    //: func stopTimer() {
    func renderDisappear() {
...
extension SinceTopView {
    ...
    func regionScreenView(_ liveModel: ErrorRoomModel) {
        ...
        uidLab.text = (String(str_imageCurrentTitle.prefix(4))) + "\(liveModel.streamerInfo.uid)"
        ...
        icon.laughDoing(urlStr: liveModel.streamerInfo.headPic, placeImg: UIImage.requestSex(sex: liveModel.streamerInfo.sex), options: optionsInfo)
        // 是否是主播
        ...
        if isAnchor {
            //: refreshLiveStatus()
            stepDown()
            //: miniBtn.setImage(UIImage.markTag(name: "btn_live_mini_nor"), for: .normal)
            miniBtn.setImage(UIImage.markTag(name: (String(str_barButtonData))), for: .normal)
            //: refreshFollow(isAttention: true)
            punctuate(isAttention: true)
            //: } else {
        } else {
            //: miniBtn.setImage(UIImage.markTag(name: "btn_live_closed_nor"), for: .normal)
            miniBtn.setImage(UIImage.markTag(name: (String(str_topTitle.suffix(5)) + String(str_toolPicName.suffix(5)) + "losed_nor")), for: .normal)
            //: refreshFollow(isAttention: self.liveModel.streamerInfo.isAttention)
            punctuate(isAttention: self.liveModel.streamerInfo.isAttention)
        }
    }

    /// 最小化/关闭直播 按钮点击事件
    //: @objc func miniButtonClick() {
    @objc func objectStatus() {
        //: rightBtnBlock?()
        rightBtnBlock?()
        // 是否是主播
        //: let isAnchor = (String(liveModel.streamerInfo.uid) == RecordAppManager.share.loginUid)
        ...
        if isAnchor { // 改为小窗口
            ...
            CellPushListener.inserter().modelBySession(smallWindow: true)
            //: currentViewController()?.navigationController?.popViewController(animated: false)
            greetAt()?.navigationController?.popViewController(animated: false)
        }
    }
    ...
    @objc private func listen() {
        ...
        activity(uid: String(CellPushListener.inserter().liveRoomModel.streamerInfo.uid))
    }

    /// 展示个人资料卡
    //: private func showUserCard(uid: String) {
    private func activity(uid: String) {
        //: let userCard = TalkingLiveUserCardView(frame: .zero, uid: uid)
        let userCard = KeyMaleCardView(frame: .zero, uid: uid)
        //: userCard.delegate = weakVC as? any TalkingLiveUserCardViewDelegate
        userCard.delegate = weakVC as? any PathObjectProtocol
        //: userCard.show()
        userCard.atMagnitude()
    }

    /// 刷新关注按钮
    //: private func refreshFollow(isAttention: Bool) {
    private func punctuate(isAttention: Bool) {
    ...
    private func stepDown() {
        ...
        self.liveStatusImgV.isHidden = !CellPushListener.inserter().isLive
    ...
    @objc private func notWindowClick() {
        //: req_followRequest(uid: liveModel.streamerInfo.uid, isAttention: liveModel.streamerInfo.isAttention) { [weak self] succeed in
        part(uid: liveModel.streamerInfo.uid, isAttention: liveModel.streamerInfo.isAttention) { [weak self] succeed in
            //: guard let self = self else { return }
            guard let self = self else { return }
            //: guard succeed else { return }
            guard succeed else { return }
            ...
            self.punctuate(isAttention: self.liveModel.streamerInfo.isAttention)
        }
    }
}
...
extension SinceTopView {
    ...
    private func someone() {
        ...
        self.frame = CGRect(x: 0, y: 0, width: kLet_itemData, height: kLet_pathErrData + actualWidth(w: 68))
