...
fileprivate let str_fatalText:[UInt8] = [0x37,0x30,0x37,0x2a,0x76,0x3d,0x31,0x3a,0x3b,0x2c,0x64,0x77,0x7e,0x36,0x3f,0x2d,0x7e,0x30,0x31,0x2a,0x7e,0x3c,0x3b,0x3b,0x30,0x7e,0x37,0x33,0x2e,0x32,0x3b,0x33,0x3b,0x30,0x2a,0x3b,0x3a]

private func scaleValue(cell num: UInt8) -> UInt8 {
    return num ^ 94
}

/*: "move" :*/
fileprivate let str_outletTitle:String = "moequale"
...
class VersionScreenView: UIView {
    ...
    var danumuMsgArray: [ResponseScreenModel] = []
    //: var danumuItemViewArray: [TalkingFloatScreenItemView] = []
    var danumuItemViewArray: [InfluenceView] = []

    //: let TapAction: PublishSubject<TalkingFloatingScreenModel?> = PublishSubject()
    let TapAction: PublishSubject<ResponseScreenModel?> = PublishSubject()
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: str_fatalText.map{scaleValue(cell: $0)}, encoding: .utf8)!)
    }
    ...
    func textTable(_ danumuMsgModel: ResponseScreenModel?) {
    ...
    @objc func largeLess(_: UITapGestureRecognizer) {}
    ...
    lazy var tapGesture: UITapGestureRecognizer = {
        ...
        let t = UITapGestureRecognizer(target: self, action: #selector(largeLess(_:)))
        //: t.delegate = self
        t.delegate = self
        //: return t
        return t
...
extension VersionScreenView: UIGestureRecognizerDelegate {
    ...
    func withConversation(for danumuMsgModel: ResponseScreenModel?, isFirst: Bool) {
        //: let itemView = TalkingFloatScreenItemView(model: danumuMsgModel)
        let itemView = InfluenceView(model: danumuMsgModel)
        ...
        if let anim = POPBasicAnimation(propertyNamed: kPOPViewFrame) {
            ...
            itemView.pop_add(anim, forKey: (str_outletTitle.replacingOccurrences(of: "equal", with: "v")))
    ...
    func daumu(_ isFirst: Bool) {
        ...
        if danumuMsgArray.count > 0 {
            ...
            if let model {
                ...
                withConversation(for: model, isFirst: isFirst)
