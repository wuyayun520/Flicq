...
/*: "Speaker On" :*/
fileprivate let str_tableAfterData:String = "Speakelet add for"

/*: "Speaker Off" :*/
fileprivate let str_behaviorTitle:String = "sex"
fileprivate let str_modelToContent:String = "ppointker"

/*: "s- :*/
fileprivate let str_pushText:[Character] = ["s","-"]

/*: - :*/
fileprivate let str_picName:String = "make"

/*: "TRTC 音频通话销毁. logId： :*/
fileprivate let str_sectionData:[Character] = ["T","R","T"]
fileprivate let str_priceText:[Character] = ["C"," ","音","频","通","话","销","毁","."," ","l","o","g","I","d","："]

/*: ." :*/
fileprivate let str_createData:String = "."

/*: "TRTC 音频通话错误. errCode: :*/
fileprivate let str_userScreenValue:String = "TRTC 音频\u{901a}话"
fileprivate let str_tipData:String = "pull make user错误. er"

/*: , errMsg: :*/
fileprivate let str_tapValue:String = ", etool else"
fileprivate let str_homeValue:String = "error guard selfrrMsg:"

/*: "TRTC 音频通话SDK收到警告. warningCode: :*/
fileprivate let str_backValue:[Character] = ["T","R","T","C"," ","音","频","通","话","S","D","K","收","到","警","告"]
fileprivate let str_countInfoValue:[Character] = ["."," ","w","a","r","n","i"]
fileprivate let str_byLikeName:[Character] = ["n","g","C","o","d","e",":"]

/*: , warningMsg: :*/
fileprivate let str_topTitle:[Character] = [","," ","w","a","r","n","i","n"]
fileprivate let str_shareData:String = "limit name with coatgMsg:"

/*: "TRTC 音频通话进入房间. result: :*/
fileprivate let str_titleEqualValue:String = "true error view selfTRTC 音"
fileprivate let str_skipName:String = "table. "
fileprivate let str_mainIndexName:String = "result:type item class nor"

/*: , logId： :*/
fileprivate let str_fullAddText:String = "net gift, logId："

/*:  加入了房间" :*/
fileprivate let str_managerFillName:String = " 加入了房check"

/*:  离开了房间" :*/
fileprivate let str_segmentData:String = " 离view房"
fileprivate let str_makeText:String = "间"
...
private class FactorReactiveCompatible: NSObject {
...
class RangeChatManager: NSObject {
    ...
    private var config = FactorReactiveCompatible() // 音频渲染配置类
    //: private var logId = 0
...
extension RangeChatManager {
    ...
    func toId(roomId: Int) {
        //: self.handsfreeBtn?.isEnabled = !hasHeadSet()
        self.handsfreeBtn?.isEnabled = !minimum()
    ...
    func displaceToMortal(isHandsfree: Bool) {
        //: guard hasHeadSet() == false else { return }
        guard minimum() == false else { return }
        ...
        self.handsfreeDesLab?.text = isHandsfree ? (String(str_tableAfterData.prefix(6)) + "r On").localized : (str_behaviorTitle.replacingOccurrences(of: "sex", with: "S") + str_modelToContent.replacingOccurrences(of: "point", with: "ea") + " Off").localized
        //: updateWhenPlayDeviceChanged(isHandsfree: isHandsfree)
        duringRemove(isHandsfree: isHandsfree)
    }
}
...
extension RangeChatManager {
    ...
    func byName() {
        ...
        NotificationCenter.default.addObserver(self,
                                               ...
                                               selector: #selector(mentalImageView(notif:)),
        ...
        NotificationCenter.default.addObserver(self,
                                               ...
                                               selector: #selector(sincePublicTransport(notif:)),
        ...
        NotificationCenter.default.addObserver(self,
                                               ...
                                               selector: #selector(enablelineWith(notif:)),
        ...
        NotificationCenter.default.addObserver(self,
                                               ...
                                               selector: #selector(changeRouteAllGiftNotifObserver(notif:)),
    ...
    @objc private func mentalImageView(notif: Notification?) {
        //: if notif == nil {
        if notif == nil {
    ...
    @objc private func sincePublicTransport(notif: Notification?) {
        //: if notif == nil {
        if notif == nil {
    ...
    @objc private func enablelineWith(notif: Notification) {
        ...
        DispatchQueue.main.async {
            ...
            if interruptStatus == AVAudioSession.InterruptionType.began {
                ...
                self.mentalImageView(notif: nil)
    ...
    @objc func changeRouteAllGiftNotifObserver(notif: Notification) {
        ...
        case .newDeviceAvailable: // 听筒
            ...
            duringRemove(isHandsfree: false)
        ...
        case .oldDeviceUnavailable: // 免提
            ...
            self.handsfreeDesLab?.text = (String(str_tableAfterData.prefix(6)) + "r On").localized
            //: updateWhenPlayDeviceChanged(isHandsfree: self.config.isHandsfree)
            duringRemove(isHandsfree: self.config.isHandsfree)
...
extension RangeChatManager {
    ...
    private func duringRemove(isHandsfree: Bool) {
    ...
    private func componentAdd() {
        //: ShowLogTool.cellText(msg: "TRTC 音频通话销毁. logId：\(logId).")
        ShowLogTool.cellText(msg: (String(str_sectionData) + String(str_priceText)) + "\(logId).")
...
extension RangeChatManager: TRTCCloudDelegate {
    //: func onError(_ errCode: TXLiteAVError, errMsg: String?, extInfo: [AnyHashable: Any]?) {
    func onError(_ errCode: TXLiteAVError, errMsg: String?, extInfo _: [AnyHashable: Any]?) {
        //: ShowLogTool.cellText(msg: "TRTC 音频通话错误. errCode:\(errCode), errMsg:\(String(describing: errMsg)).")
        ShowLogTool.cellText(msg: (str_userScreenValue + String(str_tipData.suffix(6)) + "rCode:") + "\(errCode)" + (String(str_tapValue.prefix(3)) + String(str_homeValue.suffix(6))) + "\(String(describing: errMsg)).")
        //: if errMsg != nil {
        if errMsg != nil {
    ...
    func onWarning(_ warningCode: TXLiteAVWarning, warningMsg: String?, extInfo _: [AnyHashable: Any]?) {
        //: ShowLogTool.cellText(msg: "TRTC 音频通话SDK收到警告. warningCode:\(warningCode), warningMsg:\(String(describing: warningMsg)).")
        ShowLogTool.cellText(msg: (String(str_backValue) + String(str_countInfoValue) + String(str_byLikeName)) + "\(warningCode)" + (String(str_topTitle) + String(str_shareData.suffix(5))) + "\(String(describing: warningMsg)).")
    }

    //: func onEnterRoom(_ result: Int) {
    func onEnterRoom(_ result: Int) {
        //: ShowLogTool.cellText(msg: "TRTC 音频通话进入房间. result:\(result), logId：\(logId).")
        ShowLogTool.cellText(msg: (String(str_titleEqualValue.suffix(6)) + "频通话进入" + str_skipName.replacingOccurrences(of: "table", with: "房间") + String(str_mainIndexName.prefix(7))) + "\(result)" + (String(str_fullAddText.suffix(8))) + "\(logId).")
        //: if result < 0 {
        if result < 0 { // 加入失败
    ...
    func onRemoteUserEnterRoom(_ userId: String) {
        ...
        printLog(message: "\(userId)" + (str_managerFillName.replacingOccurrences(of: "check", with: "间")))
    }

    //: func onRemoteUserLeaveRoom(_ userId: String, reason: Int) {
    func onRemoteUserLeaveRoom(_ userId: String, reason _: Int) {
        //: printLog(message: "\(userId) 离开了房间")
        printLog(message: "\(userId)" + (str_segmentData.replacingOccurrences(of: "view", with: "开了") + str_makeText.capitalized))
    }
}
