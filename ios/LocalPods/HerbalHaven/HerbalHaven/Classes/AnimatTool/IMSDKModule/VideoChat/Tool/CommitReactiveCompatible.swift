...
/*: "call_bgm" :*/
fileprivate let str_mobileText:String = "emptyl"
fileprivate let str_scaleValue:[Character] = ["l","_","b","g","m"]

/*: "call_response_bgm" :*/
fileprivate let str_tagCenterContent:[Character] = ["c","a","l","l","_"]
fileprivate let str_viewBottomData:[Character] = ["r","e","s","p","o","n","s","e","_","b","g","m"]
...
// __CLOSE_PRINT__
//
//  CommitReactiveCompatible.swift
//  MLProject
//
...
class CommitReactiveCompatible: NSObject {
    //: var type = CallBGMPlayerType.Call
    var type = BackBinaryCount.Call
    //: var player = AVPlayer()
    var player = AVPlayer()
    //: var filePath = ""
    var filePath = ""
    ...
    class func destroyType(type: BackBinaryCount) -> CommitReactiveCompatible {
        //: let player = TalkingCallBGMPlayer()
        let player = CommitReactiveCompatible()
        //: player.type = type
        player.type = type
        //: player.getFilePath()
        player.getBarPrimrosePath()
        //: return player
        return player
    }
...
extension CommitReactiveCompatible {
    //: func play() {
    func executeWith() {
    ...
    private func sectionTeeEnable(notify: Notification) {
    ...
    private func aboveRecord(notify: Notification) {
    ...
    //: func stop() {
    func analogDigitalConverterConversation() {
    ...
    private func getBarPrimrosePath() {
        //: var fileName = "call_bgm"
        var fileName = (str_mobileText.replacingOccurrences(of: "empty", with: "ca") + String(str_scaleValue))
        //: if self.type == .Response {
        if self.type == .Response {
            //: fileName = "call_response_bgm"
            fileName = (String(str_tagCenterContent) + String(str_viewBottomData))
        }
        // 收到通话推送要播放mp3声音，不能放到.zip中
        //: let path = SVGAEffectTool.default.getMp3Path(name: fileName)
        let path = EffectTool.default.blockKind(name: fileName)
        //: self.filePath = path
        self.filePath = path
    }
}
...
extension CommitReactiveCompatible: TXVodPlayListener {
