...
fileprivate let str_sectionSharedTitle:[UInt8] = [0xed,0xea,0xed,0xf0,0xac,0xe7,0xeb,0xe0,0xe1,0xf6,0xbe,0xad,0xa4,0xec,0xe5,0xf7,0xa4,0xea,0xeb,0xf0,0xa4,0xe6,0xe1,0xe1,0xea,0xa4,0xed,0xe9,0xf4,0xe8,0xe1,0xe9,0xe1,0xea,0xf0,0xe1,0xe0]

private func selfApp(error num: UInt8) -> UInt8 {
    return num ^ 132
}

/*: "#171717" :*/
fileprivate let str_infoDayData:[Character] = ["#","1","7","1","7","1","7"]

/*: "icon_cameraoff" :*/
fileprivate let str_topName:[Character] = ["i","c","o","n","_","c","a","m","e","r","a","o","f","f"]
...
// __CLOSE_PRINT__
//
//  ConditionReactiveCompatible.swift
//  AbroadTalking
//
...
class ConditionReactiveCompatible: UIView {
    ...
    override init(frame: CGRect) {
        ...
        self.backgroundColor = UIColor.quantityerfoil(51, 51, 51, 1)
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: str_sectionSharedTitle.map{selfApp(error: $0)}, encoding: .utf8)!)
    }
    ...
    lazy var bgView: UIImageView = {
        ...
        alphaV.backgroundColor = UIColor(hex: (String(str_infoDayData)))
        //: alphaV.alpha = 0.6
        alphaV.alpha = 0.6
        //: v.addSubview(alphaV)
        v.addSubview(alphaV)
    ...
    private lazy var icon: UIImageView = {
        ...
        //: v.contentMode = .scaleAspectFill
        v.contentMode = .scaleAspectFill
        //: v.image = UIImage.markTag(name: "icon_cameraoff")
        v.image = UIImage.markTag(name: (String(str_topName)))
        //: v.isHidden = true
        v.isHidden = true
        //: return v
        return v
...
extension ConditionReactiveCompatible {
    /// 刷新视频通话背景
    /// - Parameters:
    ///   - urlStr: 背景图片
    ///   - isOpenCamera: 摄像头状态
    //: func refreshBgView(urlStr: String?, isOpenCamera: VideoCameraStatus) {
    func activityCamera(urlStr: String?, isOpenCamera: ConditionCameraStatus) {
...
extension ConditionReactiveCompatible: UIGestureRecognizerDelegate {
    /// 拖拽位移手势
    /// - Parameter pan: 手势
    //: @objc private func dragWindow(pan: UIPanGestureRecognizer) {
    @objc private func lightData(pan: UIPanGestureRecognizer) {
        ...
        newCenterX = min(newCenterX, kLet_itemData - self.frame.size.width * 0.5)
        //: newCenterY = max(self.frame.size.height * 0.5, newCenterY)
        newCenterY = max(self.frame.size.height * 0.5, newCenterY)
        //: newCenterY = min(newCenterY, ScreenHeight - self.frame.size.height * 0.5)
        newCenterY = min(newCenterY, kLet_dismissValue - self.frame.size.height * 0.5)
...
extension ConditionReactiveCompatible {
    ...
    private func systemOfRules() {
        ...
        let tap = UITapGestureRecognizer(target: self, action: #selector(parameter))
        ...
        let pan = UIPanGestureRecognizer(target: self, action: #selector(lightData(pan:)))
        //: pan.delegate = self
        pan.delegate = self
