...
fileprivate let str_transactionName:[UInt8] = [0x64,0x65,0x74,0x6e,0x65,0x6d,0x65,0x6c,0x70,0x6d,0x69,0x20,0x6e,0x65,0x65,0x62,0x20,0x74,0x6f,0x6e,0x20,0x73,0x61,0x68,0x20,0x29,0x3a,0x72,0x65,0x64,0x6f,0x63,0x28,0x74,0x69,0x6e,0x69]
...
class ListTrackView: UIView {
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: str_transactionName.reversed(), encoding: .utf8)!)
    }
...
extension ListTrackView {
    ...
    override func hitTest(_ point: CGPoint, with _: UIEvent?) -> UIView? {
        ...
        for subview in self.subviews {
            ...
            let itemview: TableLabStraddleReactiveCompatible = subview as! TableLabStraddleReactiveCompatible
            ...
            if #available(iOS 11.0, *) {
                ...
                if itemview.avatarImage.frame.contains(point) {
                    //: EnvelopeSinceAdministratorThen.share.func__pushToUserDetailVC(uid: "\(itemview.giftMsgModel.fromUid ?? 0)")
                    EnvelopeSinceAdministratorThen.share.alongUid(uid: "\(itemview.giftMsgModel.fromUid ?? 0)")
                    //: break
                    break
                }
                //: } else {
    ...
    func dealModel(model: PartAnimatModel) {
        //  1.相同礼物时则连击
        //: for  subview in self.subviews {
        for subview in self.subviews {
            //: let itemview: TalkingGiftTrackItemView = subview as! TalkingGiftTrackItemView
            ...
            if openEach(giftMsgModel: itemview.giftMsgModel, withGiftMsgModel: model) {
                //: itemview.comboHitWith(giftMsgModel: model)
                itemview.socialistParty(giftMsgModel: model)
                //: itemview.setGiftMsgModel(model: model)
                itemview.observerModel(model: model)
                //: return
                return
            }
        }
        ...
        for index in 0 ... 2 {
            ...
            let subView: TableLabStraddleReactiveCompatible? = self.viewWithTag(tag) as! TableLabStraddleReactiveCompatible?
            //: if subView == nil {
            if subView == nil {
                //: self.addNewGiftDanumWith(giftMsgModel: model, atIndex: index)
                self.voiceHide(giftMsgModel: model, atIndex: index)
                //: return
                return
            }
        }
        ...
        for index in 0 ... 2 {
            ...
            if index == 1 || index == 2 {
                ...
                let subView1: TableLabStraddleReactiveCompatible = self.viewWithTag(tag) as! TableLabStraddleReactiveCompatible
    ...
    func openEach(giftMsgModel: PartAnimatModel, withGiftMsgModel: PartAnimatModel) -> (Bool) {
    ...
    func voiceHide(giftMsgModel: PartAnimatModel, atIndex: Int) {
        //: var startX = 0
        var startX = 0
        //: if LabelToReactiveCompatible.shared.direction == .rightToLeft {
        if LabelToReactiveCompatible.shared.direction == .rightToLeft {
            //: startX = -Int(ScreenWidth)-15
            startX = -Int(kLet_itemData) - 15
        }
        //: let frame = CGRect.init(x: startX, y: atIndex * 60, width: Int(ScreenWidth), height: 60)
        let frame = CGRect(x: startX, y: atIndex * 60, width: Int(kLet_itemData), height: 60)
        //: let subView =  TalkingGiftTrackItemView.init(frame: frame)
        let subView = TableLabStraddleReactiveCompatible(frame: frame)
...
//        subView.showUserInfoViewBlock   = self.showUserInfoViewBlock
        ...
        subView.observerModel(model: giftMsgModel)
        //: self.addSubview(subView)
        self.addSubview(subView)

        //: subView.showGiftAnimate(model: giftMsgModel, animate: true)
        subView.equalEmpty(model: giftMsgModel, animate: true)
    }
}
