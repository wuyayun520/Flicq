...
fileprivate let str_mainText:[UInt8] = [0x29,0x2e,0x29,0x34,0x68,0x23,0x2f,0x24,0x25,0x32,0x7a,0x69,0x60,0x28,0x21,0x33,0x60,0x2e,0x2f,0x34,0x60,0x22,0x25,0x25,0x2e,0x60,0x29,0x2d,0x30,0x2c,0x25,0x2d,0x25,0x2e,0x34,0x25,0x24]

private func regularLabel(version num: UInt8) -> UInt8 {
    return num ^ 64
}

/*: "#FFD13A" :*/
fileprivate let str_pullData:[Character] = ["#","F","F","D","1","3","A"]

/*: "gift_gb_umnad_1" :*/
fileprivate let str_markValue:String = "gift_make tag"
fileprivate let str_centerLessText:String = "title direct key view datanad_1"

/*: "icon_circle_pic_default" :*/
fileprivate let str_taskName:[Character] = ["i","c","o","n","_","c","i","r","c","l"]
fileprivate let str_leadingText:String = "var let background right sharee_pi"
fileprivate let str_keyTitleData:String = "celllt"

/*: "Open lucky blind-box" :*/
fileprivate let str_topTitle:[Character] = ["O","p","e","n"," ","l","u","c","k","y"," ","b","l","i","n","d","-","b","o","x"]

/*: "Sent " :*/
fileprivate let str_mainRoomName:String = "make let bar self makeSent "

/*: "PingFangSC-Semibold" :*/
fileprivate let str_shootName:String = "someone managerPingFa"
fileprivate let str_pointAddData:String = "makebold"

/*: "%d" :*/
fileprivate let str_fieldFreeName:String = "if collection title block%d"

/*: "Bombard" :*/
fileprivate let str_playerValue:[Character] = ["B","o","m","b","a","r"]
fileprivate let str_viewOpenlineData:String = "kind"
...
// __CLOSE_PRINT__
//
//  TableLabStraddleReactiveCompatible.swift
//  AbroadTalking
//
...
class TableLabStraddleReactiveCompatible: UIView {
    //: var giftMsgModel = TalkingGiftMsgAnimatModel()
    var giftMsgModel = PartAnimatModel()
    //: var giftSendItem = [TalkingGiftNumArrModel()]
    var giftSendItem = [InfoMeasurable()]
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: str_mainText.map{regularLabel(version: $0)}, encoding: .utf8)!)
    }
    ...
    private lazy var nickLb: UILabel = {
        ...
        //: lb.numberOfLines = 1
        lb.numberOfLines = 1
        //: lb.font = .moveDismiss(fontSize: 14)
        lb.font = .moveDismiss(fontSize: 14)
...
//        lb.numberOfLines = 1
        ...
        lb.textColor = UIColor(hex: (String(str_pullData)))
    ...
    private lazy var pluralTip: BeltItemTitleLabelAwardThen = {
        //: let lab = TalkingGiftTrackItemLabel()
        let lab = BeltItemTitleLabelAwardThen()
        //: lab.isHidden = true
        lab.isHidden = true
    ...
    private lazy var countLb: BeltItemTitleLabelAwardThen = {
        //: let lab = TalkingGiftTrackItemLabel()
        let lab = BeltItemTitleLabelAwardThen()
...
extension TableLabStraddleReactiveCompatible {
    //: func setGiftMsgModel(model: TalkingGiftMsgAnimatModel) {
    func observerModel(model: PartAnimatModel) {
        //: self.giftMsgModel = model
        self.giftMsgModel = model
        //: let fromNickname = giftMsgModel.fromNickname
        let fromNickname = giftMsgModel.fromNickname
        //: self.bgImage.image = UIImage.markTag(name: "gift_gb_umnad_1")
        self.bgImage.image = UIImage.markTag(name: (String(str_markValue.prefix(5)) + "gb_um" + String(str_centerLessText.suffix(5))))

        //: let placeholder = UIImage.markTag(name: "icon_circle_pic_default")
        let placeholder = UIImage.markTag(name: (String(str_taskName) + String(str_leadingText.suffix(4)) + "c_def" + str_keyTitleData.replacingOccurrences(of: "cell", with: "au")))

        //: if (model.showType == ChatGiftAnimatType.myStery.rawValue ) {
        if model.showType == NameEquatable.myStery.rawValue {
            //: self.nickLb.text = "Open lucky blind-box".localized
            self.nickLb.text = (String(str_topTitle)).localized
            //: }else {
        } else {
        ...
        self.avatarImage.laughDoing(urlStr: giftMsgModel.fromHeadPic, placeImg: placeholder)
        //: self.giftImage.setUrlImage(urlStr: giftMsgModel.giftPic, placeImg: placeholder)
        self.giftImage.laughDoing(urlStr: giftMsgModel.giftPic, placeImg: placeholder)

        //: self.giftName.text = "Sent ".localized + giftMsgModel.pname
        self.giftName.text = (String(str_mainRoomName.suffix(5))).localized + giftMsgModel.pname
        ...
        if sendNumber > 1 {
            ...
            var font1 = UIFont(name: (String(str_shootName.suffix(6)) + "ngSC-Se" + str_pointAddData.replacingOccurrences(of: "make", with: "mi")), size: 22)
            //: font1 = italicFontWith(angle: 15, fontSize: 18, fontName: font1!.fontName)
            font1 = colorTime(angle: 15, fontSize: 18, fontName: font1!.fontName)
            //: var font2 = UIFont(name: "PingFangSC-Semibold", size: 26)
            var font2 = UIFont(name: (String(str_shootName.suffix(6)) + "ngSC-Se" + str_pointAddData.replacingOccurrences(of: "make", with: "mi")), size: 26)
            //: font2 = italicFontWith(angle: 15, fontSize: 22, fontName: font2!.fontName)
            font2 = colorTime(angle: 15, fontSize: 22, fontName: font2!.fontName)
            ...
            numText = numText.frontParty(texts: strs, colors: colors, fonts: fonts)
        ...
        withTotal(giftCount: giftMsgModel.comboNum)
    }

    //: func refreshGiftCount(giftCount: Int) {
    func withTotal(giftCount: Int) {
        //: var font1 = UIFont(name: "PingFangSC-Semibold", size: 45)
        var font1 = UIFont(name: (String(str_shootName.suffix(6)) + "ngSC-Se" + str_pointAddData.replacingOccurrences(of: "make", with: "mi")), size: 45)
        //: font1 = italicFontWith(angle: 15, fontSize: 26, fontName: font1!.fontName)
        font1 = colorTime(angle: 15, fontSize: 26, fontName: font1!.fontName)
        //: var font2 = UIFont(name: "PingFangSC-Semibold", size: 45)
        var font2 = UIFont(name: (String(str_shootName.suffix(6)) + "ngSC-Se" + str_pointAddData.replacingOccurrences(of: "make", with: "mi")), size: 45)
        //: font2 = italicFontWith(angle: 15, fontSize: 38, fontName: font2!.fontName)
        font2 = colorTime(angle: 15, fontSize: 38, fontName: font2!.fontName)
        ...
        let colors = [UIColor.quantityerfoil(150, 77, 20, 1), UIColor.white, UIColor.quantityerfoil(150, 77, 20, 1)]
        ...
        attText = attText.frontParty(texts: strs, colors: colors, fonts: fonts)
        ...
        countLb.magnitudeimateAcross(attributedText: attText)
        ...
        countLb.setToGlassesStart(frame: CGRect(x: x, y: countLb.origin.y, width: width, height: countLb.size.height))
    }

    /// 连击
    //: func comboHitWith(giftMsgModel: TalkingGiftMsgAnimatModel) {
    func socialistParty(giftMsgModel: PartAnimatModel) {
        ...
        NSObject.cancelPreviousPerformRequests(withTarget: self, selector: #selector(withBackCount), object: nil)
        ...
        self.perform(#selector(withBackCount), afterDelay: 3.5)
    }

    /// 展示
    //: func showGiftAnimate(model: TalkingGiftMsgAnimatModel, animate: Bool) {
    func equalEmpty(model _: PartAnimatModel, animate _: Bool) {
    ...
    @objc func withBackCount() {
        //: NSObject.cancelPreviousPerformRequests(withTarget: self, selector: #selector(animateHide), object: nil)
        NSObject.cancelPreviousPerformRequests(withTarget: self, selector: #selector(withBackCount), object: nil)
        //: giftMsgModel = TalkingGiftMsgAnimatModel()
        giftMsgModel = PartAnimatModel()
...
extension TableLabStraddleReactiveCompatible {
    ...
    func colorTime(angle: CGFloat, fontSize: CGFloat, fontName: String) -> (UIFont) {
...
extension TableLabStraddleReactiveCompatible {
    ...
    private func pushPresent() {
        ...
        font = colorTime(angle: 15, fontSize: 20, fontName: font.fontName)
        ...
        let attText = attributedString.frontParty(texts: [(String(str_playerValue) + str_viewOpenlineData.replacingOccurrences(of: "kind", with: "d")).localized], colors: [UIColor.quantityerfoil(255, 232, 72, 1)], fonts: [font])
        //: pluralTip.setFrame(frame: CGRect.init(x: giftImage.right, y: 14, width: attText.size().width + 2, height: height-20))
        pluralTip.setToGlassesStart(frame: CGRect(x: giftImage.right, y: 14, width: attText.size().width + 2, height: height - 20))

        //: pluralTip.setAttributedText(attributedText: attText)
        pluralTip.magnitudeimateAcross(attributedText: attText)
        //: contentView.addSubview(pluralTip)
        contentView.addSubview(pluralTip)

        //: countLb.setFrame(frame: CGRect.init(x: giftImage.right, y: 2, width: 70, height: height))
        countLb.setToGlassesStart(frame: CGRect(x: giftImage.right, y: 2, width: 70, height: height))
