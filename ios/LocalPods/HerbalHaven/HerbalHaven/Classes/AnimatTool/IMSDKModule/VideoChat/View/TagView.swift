...
fileprivate let str_roundValue:[UInt8] = [0x64,0x65,0x74,0x6e,0x65,0x6d,0x65,0x6c,0x70,0x6d,0x69,0x20,0x6e,0x65,0x65,0x62,0x20,0x74,0x6f,0x6e,0x20,0x73,0x61,0x68,0x20,0x29,0x3a,0x72,0x65,0x64,0x6f,0x63,0x28,0x74,0x69,0x6e,0x69]

/*: "icon_mini_voice_end" :*/
fileprivate let str_socialRefreshContent:String = "icon_mcell image to center"
fileprivate let str_chartData:String = "user model modelvoice_end"

/*: "#F95151" :*/
fileprivate let str_hiddenText:[Character] = ["#","F","9","5","1","5","1"]

/*: "End of call" :*/
fileprivate let str_lastText:[Character] = ["E","n","d"," ","o","f"," ","c","a","l","l"]

/*: "img_home_shadow_icon" :*/
fileprivate let str_labelName:[Character] = ["i","m","g","_","h","o","m","e","_"]
fileprivate let str_byToTitle:String = "ssharedo"
...
class TagView: LoopLightMiniView {
	var managerOpen: Bool = true
	var closeDictionary: [AnyHashable: String] = [:]
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: str_roundValue.reversed(), encoding: .utf8)!)
    }
    ...
    override func disregardYearEndConversation() {
        ...
        super.disregardYearEndConversation()
            ...
            if (self.renderView.isHidden) && (self.renderView.convert(CGRect.zero, from: self.renderView.superview).origin.x == 77.58) {
            ...
            self.managerOpen = colorClose
            
            var rowFrameLet = pointAnimView.isPlayAnimation
                rowFrameLet = false
                rowFrameLet = !rowFrameLet
            if rowFrameLet != self.managerOpen {
                self.managerOpen = rowFrameLet
            }
            
                self.managerOpen = true
                self.managerOpen = !self.managerOpen
            return self.managerOpen
            }
            rowFrameLet.modelDictionary = { [self] methodGiftDictionary in
            self.closeDictionary = methodGiftDictionary
            
            guard var value = self.closeDictionary as? [String: String] else {
                return nil
            }
            return value
            }
                self.renderView.addSubview(rowFrameLet)
            }
    ...
    private lazy var statusIcon: UIImageView = {
        //: let img = UIImageView()
        let img = UIImageView()
        //: img.image = UIImage.markTag(name: "icon_mini_voice_end")
        img.image = UIImage.markTag(name: (String(str_socialRefreshContent.prefix(6)) + "ini_" + String(str_chartData.suffix(9))))
    ...
    private lazy var shadowImg: UIImageView = {
        ...
        img.image = UIImage.markTag(name: (String(str_labelName) + str_byToTitle.replacingOccurrences(of: "share", with: "ha") + "w_icon"))
...
extension TagView {
    ...
    func freeSpokenPoint(point: String) {
