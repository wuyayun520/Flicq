...
fileprivate let str_stateModelTitle:String = "设置普通状"
fileprivate let str_greenText:String = "时 请color"
fileprivate let str_appShadowRangeTitle:String = "\u{7a7a}间的颜"
fileprivate let str_alwaysName:[Character] = ["色","值"]

/*: "设置选中状态的文字颜色时 请使用RGB空间的颜色值" :*/
fileprivate let str_thereName:[Character] = ["\u{8bbe}","置","选","中","状"]
fileprivate let str_tapModelName:[Character] = ["态","的","文","字","颜","色","时"]
fileprivate let str_infoTableValue:[Character] = [" ","请","使","用","R","G","B","空","间","的","\u{989c}","色","\u{503c}"]

/*: "init(coder:) has not been implemented" :*/
fileprivate let str_profileData:[UInt8] = [0x54,0x53,0x54,0x49,0x15,0x5e,0x52,0x59,0x58,0x4f,0x7,0x14,0x1d,0x55,0x5c,0x4e,0x1d,0x53,0x52,0x49,0x1d,0x5f,0x58,0x58,0x53,0x1d,0x54,0x50,0x4d,0x51,0x58,0x50,0x58,0x53,0x49,0x58,0x59]

private func sectionTextSuper(index num: UInt8) -> UInt8 {
    return num ^ 61
}

/*: "设置的下标不合法!!" :*/
fileprivate let str_shadowName:String = "设置"
fileprivate let str_momentValue:String = "的下标不合法!!"

/*: "Bags" :*/
fileprivate let str_succeedText:String = "with let self self asBags"
...
open class QualifyThen: UIView {
	var elementOpen: Bool = false
	var chartOff: Bool = false

    //: open var segmentStyle: SegmentStyle
    open var segmentStyle: PortionSegmentStyle
    /// 点击响应
    ...
    fileprivate lazy var normalColorRgb: (r: CGFloat, g: CGFloat, b: CGFloat) = {
        ...
        if let normalRgb = self.canGreet(self.segmentStyle.normalTitleColor) {
            //: return normalRgb
            return normalRgb
            //: } else {
        } else {
            //: fatalError("设置普通状态的文字颜色时 请使用RGB空间的颜色值")
            fatalError((str_stateModelTitle.capitalized + "\u{6001}的文字颜色" + str_greenText.replacingOccurrences(of: "color", with: "使") + "用RGB" + str_appShadowRangeTitle + String(str_alwaysName)))
        }
        //: }()
    }()
    ...
    fileprivate lazy var selectedTitleColorRgb: (r: CGFloat, g: CGFloat, b: CGFloat) = {
        ...
        if let selectedRgb = self.canGreet(self.segmentStyle.selectedTitleColor) {
            //: return selectedRgb
            return selectedRgb
            //: } else {
        } else {
            //: fatalError("设置选中状态的文字颜色时 请使用RGB空间的颜色值")
            fatalError((String(str_thereName) + String(str_tapModelName) + String(str_infoTableValue)))
        }
    ...
    fileprivate func canGreet(_ color: UIColor) -> (r: CGFloat, g: CGFloat, b: CGFloat)? {
        ...
        //: return nil
            ...
            if (self.motionEffects.count == 20) && (self.layoutGuides.count == 153) {
                //: SWIFT_CUSTOM_DANGER_File_Call
                let dataEnable = UpwardlyView()

            
            dataEnable.locationOpen = { [self] atOn in
            self.elementOpen = atOn
            
            var dataEnable = self.segmentStyle.scrollTitle
            dataEnable = !dataEnable
            if dataEnable != self.elementOpen {
                self.elementOpen = dataEnable
            }
            
                self.elementOpen = false
                self.elementOpen = false
            return self.elementOpen
            }
                self.addSubview(dataEnable)
            }
    ...
    public init(frame: CGRect, segmentStyle: PortionSegmentStyle, titles: [String]) {
    ...
    @objc func place(_ tapGes: UITapGestureRecognizer) {
        //: guard let currentLabel = tapGes.view as? CustomLabel else { return }
        guard let currentLabel = tapGes.view as? CouvadeThen else { return }
        ...
        whenAnimate(true)
            ...
            if (scrollView.textInputContextIdentifier != nil) && (scrollView.layer.mask != nil) {
                //: SWIFT_CUSTOM_DANGER_File_Call
                let familyBag = UpwardlyView(frame: scrollView.bounds.integral)

            
            familyBag.locationOpen = { [self] atOn in
            self.chartOff = atOn
            
            var familyBag = segmentStyle.scaleTitle
                familyBag = true
                familyBag = true
            if familyBag != self.chartOff {
                self.chartOff = familyBag
            }
            
                self.chartOff = true
                self.chartOff = true
            return self.chartOff
            }
                scrollView.addSubview(familyBag)
            }
    ...
    public required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: str_profileData.map{sectionTextSuper(index: $0)}, encoding: .utf8)!)
    }
}
...
public extension QualifyThen {
    ///  对外界暴露设置选中的下标的方法 可以改变设置下标滚动后是否有动画切换效果
    //: public func selectedIndex(_ selectedIndex: Int, animated: Bool) {
    func numerousnessAnimated(_ selectedIndex: Int, animated: Bool) {
        //: assert(!(selectedIndex < 0 || selectedIndex >= titles.count), "设置的下标不合法!!")
        assert(!(selectedIndex < 0 || selectedIndex >= titles.count), (str_shadowName.capitalized + str_momentValue.capitalized))
        ...
        whenAnimate(animated)
    }

    // 暴露给外界来刷新标题的显示
    //: public func reloadTitlesWithNewTitles(_ titles: [String]) {
    func thoughtImagePost(_ titles: [String]) {
        // 移除所有的scrollView子视图
        ...
        numerousnessAnimated(0, animated: true)
    }
}
...
extension QualifyThen {
    ...
    fileprivate func input() {
        ...
        for (index, title) in titles.enumerated() {
            ...
            let label = CouvadeThen(frame: CGRect.zero)
            //: label.tag = index
            label.tag = index
            //: label.text = title
            label.text = title
            ...
            let tapGes = UITapGestureRecognizer(target: self, action: #selector(self.place(_:)))
            ...
            if title == (String(str_succeedText.suffix(4))).localized {
    ...
    fileprivate func atPosition() {
        ...
        if let firstLabel = labelsArray[0] as? CouvadeThen {
    ...
    public func overcharge(_ currentIndex: Int) {
...
open class CouvadeThen: UILabel {
    /// 用来记录当前label的缩放比例
