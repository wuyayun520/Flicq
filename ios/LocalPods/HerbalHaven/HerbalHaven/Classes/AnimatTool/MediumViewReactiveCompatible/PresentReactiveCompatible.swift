...
fileprivate let str_actionTitle:[UInt8] = [0x93,0x98,0x93,0x9e,0x52,0x8d,0x99,0x8e,0x8f,0x9c,0x64,0x53,0x4a,0x92,0x8b,0x9d,0x4a,0x98,0x99,0x9e,0x4a,0x8c,0x8f,0x8f,0x98,0x4a,0x93,0x97,0x9a,0x96,0x8f,0x97,0x8f,0x98,0x9e,0x8f,0x8e]

fileprivate func addReturn(top num: UInt8) -> UInt8 {
...
/*: "Reply" :*/
fileprivate let str_shadowMagnitudeerestName:String = "Replystring manager"

/*: "#F95151" :*/
fileprivate let str_clickText:String = "#F9515user value else"
fileprivate let str_alongText:[Character] = ["1"]
...
let kLet_userMessageTotalTitle = (kLet_pathErrData + 80.0)
//: class TalkingMessageNotificationView: UIView {
class PresentReactiveCompatible: UIView {
    //: var startDismissBlock: ((_ model: TalkingConversationModel) -> Void)?
    var startDismissBlock: ((_ model: DistinctConversationModel) -> Void)? // 开始移除视图Block
    //: var finishDismissBlock: ((_ view: TalkingMessageNotificationView) -> Void)?
    var finishDismissBlock: ((_ view: PresentReactiveCompatible) -> Void)? // 完成移除视图Block
    //: private var currModel = TalkingConversationModel()
    private var currModel = DistinctConversationModel()
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: str_actionTitle.map{addReturn(top: $0)}, encoding: .utf8)!)
    }
    ...
    private lazy var nickNameLabel: UILabel = {
        ...
        let label = UILabel()
        //: label.font = .biologyLaboratoryEqual(type: .Medium, fontSize: 15)
        label.font = .biologyLaboratoryEqual(type: .Medium, fontSize: 15)
    ...
    private lazy var replyBtn: UIButton = {
        //: let btn = UIButton()
        let btn = UIButton()
        //: btn.titleLabel?.font = .biologyLaboratoryEqual(type: .Medium, fontSize: 15)
        btn.titleLabel?.font = .biologyLaboratoryEqual(type: .Medium, fontSize: 15)
        //: btn.setTitle("Reply".localized, for: .normal)
        btn.setTitle((String(str_shadowMagnitudeerestName.prefix(5))).localized, for: .normal)
        //: btn.setTitleColor(UIColor(hex: "#F95151"), for: .normal)
        btn.setTitleColor(UIColor(hex: (String(str_clickText.prefix(6)) + String(str_alongText))), for: .normal)
        //: btn.isEnabled = false
        btn.isEnabled = false
        //: return btn
        return btn
...
extension PresentReactiveCompatible {
    /// 展示视图，5s后自动移除
    //: func show(_ model: TalkingConversationModel?) {
    func doAdd(_ model: DistinctConversationModel?) {
    ...
    func second(needAlpha: Bool = true) {
        ...
        DispatchQueue.main.async {
            ...
            UIView.animate(withDuration: 0.5) {
                ...
                if needAlpha {
                    ...
                    self.frame.origin.y = -kLet_userMessageTotalTitle
                }
                //: } completion: { _ in
            } completion: { _ in
    ...
    @objc private func indexMovement() {
        ...
        if currModel.chatType == .service {
            ...
            //: EnvelopeSinceAdministratorThen.share.func__pushToPriveteChatVC(chatID: currModel.targetId)
            EnvelopeSinceAdministratorThen.share.hiddenCompletion(chatID: currModel.targetId)
        }
    }

    /// 滑动手势
    //: @objc private func swipeGestureEvent(gesture: UISwipeGestureRecognizer) {
    @objc private func barroom(gesture: UISwipeGestureRecognizer) {
...
extension PresentReactiveCompatible {
    ...
    func imageDevice() {
        ...
        if let userInfo = currModel.gj_userInfo {
            ...
            headImgView.laughDoing(urlStr: userInfo.headPic, placeImg: UIImage.requestSex(sex: userInfo.sex))
            //: if !userInfo.headPicFrame.isEmptyString {
            if !userInfo.headPicFrame.isEmptyString {
                //: iconBorder.isHidden = false
                iconBorder.isHidden = false
                //: iconBorder.setHeadFrameUrlImage(urlStr: userInfo.headPicFrame)
                iconBorder.glasses(urlStr: userInfo.headPicFrame)
                //: } else {
            } else {
            ...
            headImgView.image = UIImage.overSize()
            //: iconBorder.isHidden = true
            iconBorder.isHidden = true
        }

        //: msgShowLabel.attributedText = TalkingChatListTableCell.handleShowC2CDetailTextOfMessage(model: currModel)
        msgShowLabel.attributedText = FileAllocationTableUserReactiveCompatible.sectionIcon(model: currModel)
    }
    ...
    private func readOut() {
        ...
        self.frame = CGRect(x: 0, y: -kLet_userMessageTotalTitle, width: kLet_itemData, height: kLet_userMessageTotalTitle)
    ...
    private func pedestal() {
        ...
        let tap = UITapGestureRecognizer(target: self, action: #selector(indexMovement))
        //: self.addGestureRecognizer(tap)
        self.addGestureRecognizer(tap)

        //: let swipe = UISwipeGestureRecognizer(target: self, action: #selector(swipeGestureEvent))
        let swipe = UISwipeGestureRecognizer(target: self, action: #selector(barroom))
        //: swipe.direction = .up
        swipe.direction = .up
