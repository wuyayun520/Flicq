...
/*: "gifts" :*/
fileprivate let str_premiumTitle:[Character] = ["g","i","f","t","s"]

/*: "vipGifts" :*/
fileprivate let str_layerData:String = "if do labvipGi"
fileprivate let str_reasonShadowName:String = "fpic"

/*: "packGifts" :*/
fileprivate let str_labGiftTitle:[Character] = ["p","a","c"]
fileprivate let str_sendSizeData:[Character] = ["k","G","i","f","t","s"]

/*: "giftSendItem" :*/
fileprivate let str_modelMessageData:String = "return image error infogiftSen"
fileprivate let str_ofLocationName:String = "model instance model icondItem"
...
class EqualGiftManager: NSObject {
    /// 是否展示背包红点
    ...
    private var cacheDict = [String: [BackgroundWidthTotal: [Any]]]()
    // 数据锁
    ...
    private var giftType = RecordValueConvertible.normal

    //: static let share = TalkingChatGiftManager()
    static let share = EqualGiftManager()
...
extension EqualGiftManager {
    ...
    func panoramicViewCompletion(type: RecordValueConvertible = .normal, isRefresh: Bool = false, completion: @escaping () -> Void) {
        //: TalkingChatGiftManager.share.giftType = type
        EqualGiftManager.share.giftType = type
        //: if isRefresh == false {
        if isRefresh == false {
            //: let dataDict = cacheDict[type.rawValue]
            let dataDict = cacheDict[type.rawValue]
            //: let hotArr = dataDict?[GiftDataType.Hot] ?? [Any]()
            let hotArr = dataDict?[BackgroundWidthTotal.Hot] ?? [Any]()
        ...
        if type == .groupChat {
            ...
            EqualGiftManager.share.readingCompletion(completion: { isSucceed in
            ...
            EqualGiftManager.share.aggregationOff(completion: { isSucceed in
                //: guard isSucceed else { return }
                guard isSucceed else { return }
                //: let dataDict = self.cacheDict[type.rawValue]
                let dataDict = self.cacheDict[type.rawValue]
            ...
            EqualGiftManager.share.writerSTitle(completion: { isSucceed in
                //: guard isSucceed else { return }
                guard isSucceed else { return }
                //: let dataDict = self.cacheDict[type.rawValue]
                let dataDict = self.cacheDict[type.rawValue]
            ...
            EqualGiftManager.share.fullMoon(completion: { isSucceed in
                //: guard isSucceed else { return }
                guard isSucceed else { return }
                //: let dataDict = self.cacheDict[type.rawValue]
                let dataDict = self.cacheDict[type.rawValue]
    ...
    private func fullMoon(completion: @escaping (_ isSucceed: Bool) -> Void) {
        //: let type = TalkingChatGiftManager.share.giftType
        let type = EqualGiftManager.share.giftType
        //: LiveRowRequestTool.req_ChatGiftList(type: type.rawValue) { succeed, result, errorModel in
        LiveRowRequestTool.blockAdd(type: type.rawValue) { succeed, result, _ in
            //: if succeed {
            if succeed {
                //: let data: Dictionary = result as! Dictionary<String, Any>
                let data: Dictionary = result as! [String: Any]
                //: var dataDict = [GiftDataType: [Any]]()
                var dataDict = [BackgroundWidthTotal: [Any]]()
                // hot礼物
                //: if let array: Array = data["gifts"] as? Array<Any> {
                if let array: Array = data[(String(str_premiumTitle))] as? [Any] {
                    //: var hotArr = [Any]()
                    var hotArr = [Any]()
                    //: if let datas = Array<TalkingRoomGiftModel>.deserialize(from: array as? Array) {
                    if let datas = Array<RecordTransformable>.deserialize(from: array as? Array) {
                        //: hotArr.append(contentsOf: datas as [Any])
                        hotArr.append(contentsOf: datas as [Any])
                    }
                    //: dataDict[GiftDataType.Hot] = hotArr
                    dataDict[BackgroundWidthTotal.Hot] = hotArr
                }

                // vip礼物
                //: if let array: Array = data["vipGifts"] as? Array<Any> {
                if let array: Array = data[(String(str_layerData.suffix(5)) + str_reasonShadowName.replacingOccurrences(of: "pic", with: "ts"))] as? [Any] {
                    //: var vipArr = [Any]()
                    var vipArr = [Any]()
                    //: if let datas = Array<TalkingRoomGiftModel>.deserialize(from: array as? Array) {
                    if let datas = Array<RecordTransformable>.deserialize(from: array as? Array) {
                    ...
                    dataDict[BackgroundWidthTotal.Vip] = vipArr
                }

                // Bags礼物
                //: if let array: Array = data["packGifts"] as? Array<Any> {
                if let array: Array = data[(String(str_labGiftTitle) + String(str_sendSizeData))] as? [Any] {
                    //: var bagsArr = [Any]()
                    var bagsArr = [Any]()
                    //: if let datas = Array<TalkingRoomGiftModel>.deserialize(from: array as? Array) {
                    if let datas = Array<RecordTransformable>.deserialize(from: array as? Array) {
                    ...
                    dataDict[BackgroundWidthTotal.Bags] = bagsArr
                }
                ...
                let sendItem: Array = data[(String(str_modelMessageData.suffix(7)) + String(str_ofLocationName.suffix(5)))] as! [Any]
                ...
                for i in 0 ..< sendItem.count {
                    ...
                    if let configModel = JSONDeserializer<InfoMeasurable>.deserializeFrom(dict: sendItem[i] as? [String: Any], designatedPath: nil) {
    ...
    private func readingCompletion(completion: @escaping (_ isSucceed: Bool) -> Void) {
        //: let type = TalkingChatGiftManager.share.giftType
        let type = EqualGiftManager.share.giftType
        //: LiveRowRequestTool.req_ChatRoomGiftList(type: type.rawValue) { succeed, result, errorModel in
        LiveRowRequestTool.completion(type: type.rawValue) { succeed, result, _ in
            //: if succeed {
            if succeed {
                //: let data: Dictionary = result as! Dictionary<String, Any>
                let data: Dictionary = result as! [String: Any]
    ...
    func upTo(params: [String: Any], completion: @escaping (_ data: [Any]?) -> Void) {
        //: LiveRowRequestTool.req_getChatRoomMembers(params: params, completion: { succeed, result, errorModel in
        LiveRowRequestTool.filter(params: params, completion: { succeed, result, _ in
            //: if succeed {
            if succeed {
                ...
                for dict in data {
                    ...
                    if let memberModel = JSONDeserializer<TopMeasurable>.deserializeFrom(dict: dict as? [String: Any], designatedPath: nil) {
    ...
    private func aggregationOff(completion: @escaping (_ isSucceed: Bool) -> Void) {
        //: let type = GiftViewStyle.live
        let type = RecordValueConvertible.live
        //: LiveRowRequestTool.req_LiveGiftList(type: type.rawValue) { succeed, result, errorModel in
        LiveRowRequestTool.giftScale(type: type.rawValue) { succeed, result, _ in
            //: if succeed {
            if succeed {
                //: let data: Dictionary = result as! Dictionary<String, Any>
                let data: Dictionary = result as! [String: Any]
    ...
    private func writerSTitle(completion: @escaping (_ isSucceed: Bool) -> Void) {
        //: let type = GiftViewStyle.party
        let type = RecordValueConvertible.party
        //: LiveRowRequestTool.req_partyGiftList(type: type.rawValue) { succeed, result, errorModel in
        LiveRowRequestTool.upOrganization(type: type.rawValue) { succeed, result, _ in
            //: if succeed {
            if succeed {
                //: let data: Dictionary = result as! Dictionary<String, Any>
                let data: Dictionary = result as! [String: Any]
    ...
    func borderType(perPageCount: Int, dataType: BackgroundWidthTotal) -> Int {
    ...
    func nowType(dataType: BackgroundWidthTotal) -> [Any] {
        //: cacheLock.lock()
        cacheLock.lock()
        //: defer {
        defer {
    ...
    func liveOf() -> [Any] {
        ...
        if EqualGiftManager.share.giftType == .groupChat {
            //: return chatRoomSendItems
            return chatRoomSendItems
            //: } else if TalkingChatGiftManager.share.giftType == .live || TalkingChatGiftManager.share.giftType == .party {
        } else if EqualGiftManager.share.giftType == .live || EqualGiftManager.share.giftType == .party {
    ...
    func climax(dataType: BackgroundWidthTotal, dataArr: [Any]) {
        //: cacheLock.lock()
        cacheLock.lock()
        //: defer {
        defer {
...
extension EqualGiftManager {
    ...
    private func everyStart() {
        ...
        NotificationCenter.default.addObserver(self,
                                               ...
                                               selector: #selector(receivePackage),
                                               //: name: GIFT_REFRESHPACKAGE_NOTIFICATION,
                                               name: kLet_okCornerName,
                                               //: object: nil)
                                               object: nil)
    }
    ...
    @objc private func receivePackage() {
        //: gift_clearMemoryCache(showRed: true)
        pathRed(showRed: true)
    }
}
...
extension EqualGiftManager {
    ...
    func pathRed(showRed: Bool = false) {
        //: TalkingChatGiftManager.share.showBagsRed = showRed
        EqualGiftManager.share.showBagsRed = showRed
        //: cacheDict.removeAll()
        cacheDict.removeAll()
    }
}
