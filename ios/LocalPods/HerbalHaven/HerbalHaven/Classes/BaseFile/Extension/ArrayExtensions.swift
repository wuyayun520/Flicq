...
public class CompartmentSafeArray<Element> {
...
public extension CompartmentSafeArray {
    //: var first: Element? {
    var first: Element? {
        //: var result: Element?
        var result: Element?
...
public extension CompartmentSafeArray {
    ...
    func filter(isIncluded: @escaping (Element)->Bool) ->CompartmentSafeArray<Element> {
        //: pthread_rwlock_rdlock(&lock)
        pthread_rwlock_rdlock(&lock)
        ...
        return CompartmentSafeArray(array: result)
    }
    ...
    func sorted(by areInIncreasingOrder: (Element, Element)->Bool)->CompartmentSafeArray<Element> {
        //: pthread_rwlock_rdlock(&lock)
        pthread_rwlock_rdlock(&lock)
        //: let result = array.sorted(by: areInIncreasingOrder)
        let result = array.sorted(by: areInIncreasingOrder)
        //: let newArray = ThreadSafeArray<Element>(array: result)
        let newArray = CompartmentSafeArray<Element>(array: result)
        //: pthread_rwlock_unlock(&lock)
        pthread_rwlock_unlock(&lock)
        //: return newArray
        return newArray
...
public extension CompartmentSafeArray where Element: Equatable {
...
public extension CompartmentSafeArray {
    ...
    static func +=(left: inout CompartmentSafeArray, right: Element) {
        //: left.append(right)
        left.append(right)
    }
