...
/*: "recharge" :*/
fileprivate let str_appearTitle:String = "recsend"

/*: "recharge_ :*/
fileprivate let str_managerText:[Character] = ["r","e","c","h","a","r","g","e","_"]

/*: ".png" :*/
fileprivate let str_featureData:String = "cut count make.png"

/*: "_ :*/
fileprivate let str_labelName:String = "_"

/*: .png" :*/
fileprivate let str_reportName:String = ".pngsize allow"

/*: ".jpg" :*/
fileprivate let str_logUserContent:String = "path in view.jpg"

/*: .jpg" :*/
fileprivate let str_errorText:String = ".jpgback print"
...
class EveningShirtFixingTalkingButton: UIButton {
    ...
    var imageAlignment: SuggestCount = .left
...
extension UIButton {
    ...
    func ballsUp(_ interval: CGFloat) {
        //: verticalAlignImageAndTitle(interval: interval, topOffset: 0)
        rowTo(interval: interval, topOffset: 0)
    }
    ...
    func rowTo(interval: CGFloat, topOffset: CGFloat) {
    ...
    func arrayDetail(interval: CGFloat, imageSize: CGSize, topOffset: CGFloat) {
...
extension UIButton {
    ...
    func viewState(color: UIColor, forState: UIControl.State) {
    ...
    func failFinish(urlStr: String,
                    //: placeImg: UIImage? = UIImage.placeImgSquare(),
                    placeImg: UIImage? = UIImage.actionSize(),
                    ...
                    loadFinish: ((_ isSucceed: Bool) -> Void)? = nil)
    {
        ...
        let imgUrl = urlStr.replacingOccurrences(of: UrlDomin.urlAes256Decrypt(), with: kLet_keyViewName)
    ...
    func glasses(urlStr: String) {
        //: guard !urlStr.isEmptyString else { return }
        guard !urlStr.isEmptyString else { return }
        //: var newUrl = ""
        var newUrl = ""
        //: if urlStr.contains("recharge") {
        if urlStr.contains((str_appearTitle.replacingOccurrences(of: "send", with: "h") + "arge")) {
            //: newUrl = urlStr.replacingOccurrences(of: "recharge", with: "recharge_\(PackageID)")
            newUrl = urlStr.replacingOccurrences(of: (str_appearTitle.replacingOccurrences(of: "send", with: "h") + "arge"), with: (String(str_managerText)) + "\(kLet_closedData)")
            //: self.setUrlImage(urlStr: newUrl, placeImg: nil)
            self.failFinish(urlStr: newUrl, placeImg: nil)
            //: return
            return
                //: } else if urlStr.contains(".png") {
        } else if urlStr.contains((String(str_featureData.suffix(4)))) {
            //: newUrl = urlStr.replacingOccurrences(of: ".png", with: "_\(PackageID).png")
            newUrl = urlStr.replacingOccurrences(of: (String(str_featureData.suffix(4))), with: "_\(kLet_closedData)" + (String(str_reportName.prefix(4))))

            //: } else if urlStr.contains(".jpg") {
        } else if urlStr.contains((String(str_logUserContent.suffix(4)))) {
            //: newUrl = urlStr.replacingOccurrences(of: ".jpg", with: "_\(PackageID).jpg")
            newUrl = urlStr.replacingOccurrences(of: (String(str_logUserContent.suffix(4))), with: "_\(kLet_closedData)" + (String(str_errorText.prefix(4))))
        }
        //: self.setUrlImage(urlStr: newUrl, placeImg: nil)
        self.failFinish(urlStr: newUrl, placeImg: nil)
    }
