...
#import "UIImage+Minimise.h"

//: @implementation UIImage (Compress)
@implementation UIImage (Minimise)

//: - (NSData *)compressedImageData {
- (NSData *)creatureData {

    //: UIImage *image = self.wcTimelineCompress;
    UIImage *image = self.call;
...
- (UIImage *)betweenKind {
    ...
    return [UIImage imageWithData:[self creatureData]];
}
...
- (UIImage *)key:(Boolean)isSession {
    //: CGSize size = [self wxImageSize:isSession];
    CGSize size = [self length:isSession];
    //: UIImage *reImage = [self resizedImage:size];
    UIImage *reImage = [self keyGift:size];
...
- (CGSize)length:(Boolean)isSession {
...
+ (NSData *)consultSize:(UIImage *)image distance:(NSInteger)fileSize{
    ...
    if (shortSize>1080) {//压缩到最短边不大于1080,也避免了shortSize==0的情况
        ...
        image = [image kind:targetSize];
    }

    //: UIImage *thumIMG = image;
    UIImage *thumIMG = image;
    //: NSData *outIMGData = [thumIMG compressToFileSize:(fileSize*1024)];
    NSData *outIMGData = [thumIMG totalRelation:(fileSize*1024)];
    ...
    while (outIMGData.length>(fileSize*1024)) {
        ...
        thumIMG = [thumIMG to:scalesize currentScale:NO];

        //: outIMGData = [thumIMG compressToFileSize:(fileSize*1024)];
        outIMGData = [thumIMG totalRelation:(fileSize*1024)];
    }
...
- (UIImage*)kind:(CGSize)targetSize{
...
- (UIImage *)to:(CGSize)targetSize currentScale:(BOOL)isScale{
...
- (NSData *)totalRelation:(NSInteger)fileSize{
...
- (UIImage *)keyGift:(CGSize)newSize {
