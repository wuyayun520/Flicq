...
/*: "add_header_cookies" :*/
fileprivate let str_myFileData:[Character] = ["a","d","d","_","h","e","a","d","e","r","_","c","o","o"]
fileprivate let str_centerTitle:[Character] = ["k","i","e","s"]
...
public extension UIImageView {
    ...
    @objc func sitImageDoing(urlStr: String,
    ...
    internal func laughDoing(urlStr: String,
                             //: placeImg: UIImage = UIImage.placeImgSquare(),
                             placeImg: UIImage = UIImage.actionSize(),
    ...
    @objc func viewAction(urlStr: String) {
        //: let imgUrl = urlStr.replacingOccurrences(of: UrlDomin.urlAes256Decrypt(), with: ReplaceUrlDomain)
        let imgUrl = urlStr.replacingOccurrences(of: UrlDomin.urlAes256Decrypt(), with: kLet_keyViewName)
        //: self.kf.setImage(with: URL(string: imgUrl), placeholder: UIImage.giftPlaceHolderImage())
        self.kf.setImage(with: URL(string: imgUrl), placeholder: UIImage.imageInsert())
    }
    ...
    @objc func biscuit(urlStr: String, completion: ((_ image: UIImage?) -> Void)? = nil) {
        // 允许请求携带cookie
        ...
        if var urlComponents = URLComponents(string: urlStr),
           ...
           let currURL = URL(string: DemandReactiveCompatible.share.serverUrlStr)
        {
        ...
        let downloader = ImageDownloader(name: (String(str_myFileData) + String(str_centerTitle)))
        ...
        self.kf.setImage(with: URL(string: newImgUrl), placeholder: UIImage.actionSize(), options: optionsInfo) { result in
            //: if completion != nil {
            if completion != nil {
                //: let image = try? result.get().image
                let image = try? result.get().image
