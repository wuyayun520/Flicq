...
/*: "http://stat. :*/
fileprivate let str_rowTitle:[Character] = ["h","t","t","p",":","/","/","s","t","a","t","."]

/*: .com" :*/
fileprivate let str_yearName:[Character] = [".","c","o","m"]

/*: "http://" :*/
fileprivate let str_layerValue:[Character] = ["h","t","t","p",":"]
fileprivate let str_pictureTitle:String = "//"

/*: "https://" :*/
fileprivate let str_editContent:String = "htttime"
fileprivate let str_biologyName:String = "://"

/*: "http://m. :*/
fileprivate let str_buttonAtData:String = "devicettp"
fileprivate let str_replyTitle:[Character] = [":","/","/","m","."]

/*: "http:// :*/
fileprivate let str_touchFrontText:[Character] = ["h","t","t","p",":","/"]
fileprivate let str_hideContent:String = "/"

/*: "-1" :*/
fileprivate let str_giftCoverValue:String = "-1"

/*: "3,90" :*/
fileprivate let str_tapText:String = "3,90"

/*: "1990-01-01" :*/
fileprivate let str_circleTitle:[Character] = ["1","9","9","0","-","0"]
fileprivate let str_underText:String = "look-0look"

/*: "MM-dd-yyyy" :*/
fileprivate let str_searchServerName:String = "type addMM-"
fileprivate let str_mText:String = "if self returndd-yyyy"

/*: "id" :*/
fileprivate let str_infoModeViewData:[Character] = ["i","d"]
...
class PtolemaicSystemThen: NSObject {
    ...
    var requestType: CellNameRequestType?
    ...
    override init() {
        ...
        self.requestServer = DemandReactiveCompatible.share.serverUrlStr
...
struct MaleTransformable: HandyJSON {
    //: var errno: Int!
...
struct RangeConfigModel: HandyJSON {
    ...
    var CSConfig = PriapicModelType() // 私聊官方客服提示消息模型
//    var jumpRandomVideo: Bool = false       // true表示可以跳转到随机视频匹配
    ...
    var reportDomain: String {
        ...
        if statDomain.isEmptyString {
            ...
            return (String(str_rowTitle)) + "\(kLet_keyViewName)" + (String(str_yearName))
            //: } else {
        } else {
            //: if !statDomain.hasPrefix("http://") && !statDomain.hasPrefix("https://") {
            if !statDomain.hasPrefix((String(str_layerValue) + str_pictureTitle.capitalized)) && !statDomain.hasPrefix((str_editContent.replacingOccurrences(of: "time", with: "ps") + str_biologyName.capitalized)) {
                //: return "http://".appending(statDomain)
                return (String(str_layerValue) + str_pictureTitle.capitalized).appending(statDomain)
                //: }else {
            } else {
                //: return statDomain
                return statDomain
    ...
    var urlH5Domain: String {
        ...
        if urlDomain.isEmpty {
            ...
            return (str_buttonAtData.replacingOccurrences(of: "device", with: "h") + String(str_replyTitle)) + "\(kLet_keyViewName)" + (String(str_yearName))
            //: } else {
        } else {
            //: return "http://\(urlDomain)"
            return (String(str_touchFrontText) + str_hideContent.capitalized) + "\(urlDomain)"
        }
    }
...
public class MakeTransformable: NSObject, HandyJSON {
    ...
    var chatPriceSettings: [FocusMeasurable] = [] // 收费设置
    //: var videoPriceSettings: Array<ChatPriceModel> = []
    var videoPriceSettings: [FocusMeasurable] = [] // 视频收费设置
    //: var voicePriceSettings: Array<ChatPriceModel> = []
    var voicePriceSettings: [FocusMeasurable] = [] // 语音收费设置
    ...
    var vipSetting = [ItemRowModelType]() // vip皮肤
    ...
    var tabPageBanner = [CommitModelType]() // V1.6.4 一级页面广告
    ...
    var intimateLimit = ContextMeasurable() // 1.7.6 各种操作的亲密度限制
...
@objcMembers public class ResultTransformable: NSObject, HandyJSON {
    //: required public override init() {
    override public required init() {}
    ...
    var interest: [PathMeasurable]?
    //: var aboutMe: Array<UserSeleteTagModel>?
    var aboutMe: [PathMeasurable]?
    //: var partner: String?
    var partner: String?
    //: var full: Int?
    var full: Int?
    ...
    var gallery: [ErrorTransformable]?
    //: var status: Int = -1
    ...
    var callPopupSetting = (str_tapText.capitalized) // 表示来电弹窗配置，如”3,90”表示3s后弹第一次，后续90s，返回”0,0”表示不弹窗
    //: var callTabSwitch = 0
    ...
    var residualInfo = [PtolemaicSystemBalloonHandyJSON]() // 套餐数量

    //: func updateResidualNum(type: Int, num: Int) {
    func specialise(type: Int, num: Int) {
    ...
    public lazy var age: String = {
        ...
        let birthdayDate = NSDate.twine(self.birthday ?? (String(str_circleTitle) + str_underText.replacingOccurrences(of: "look", with: "1")), dateFormat: (String(str_searchServerName.suffix(3)) + String(str_mText.suffix(7))))
        //: guard let currentYear = NSDate.getCurrentDateComponents().year else { return "0" }
        guard let currentYear = NSDate.removeComponents().year else { return "0" }
...
struct PtolemaicSystemBalloonHandyJSON: HandyJSON {
    //: var type = 0
...
struct ErrorTransformable: HandyJSON {
    //: var id: String?
    var id: String?
    //: var url: String?
    var url: String?
...
public struct PathMeasurable: HandyJSON, Equatable {
    //: var tag_id: String?
    var tag_id: String?
    //: var tag_name = ""
    var tag_name = ""
    ...
    func leadership(compareTo: PathMeasurable) -> Bool {
        //: return
        return
...
struct TagLiveHandyJSON: HandyJSON {
    //: var aboutMe: Array<UserTagTypeModel>!
    var aboutMe: [SinceHandyJSON]!
    //: var interest: Array<UserTagTypeModel>!
    var interest: [SinceHandyJSON]!
}

//: struct UserTagTypeModel: HandyJSON {
struct SinceHandyJSON: HandyJSON {
    //: var name = ""
    var name = ""
    //: var name_ar = ""
    var name_ar = ""
    ...
    var list: [PathMeasurable]!
}

/// 官方客服提示消息模型
//: struct CSConfigModel: HandyJSON {
struct PriapicModelType: HandyJSON {
...
struct ItemRowModelType: HandyJSON {
...
struct ContextMeasurable: HandyJSON {
    //: var sendImg = 10
    var sendImg = 10 // 发送图片
    //: var sendVideo = 10
