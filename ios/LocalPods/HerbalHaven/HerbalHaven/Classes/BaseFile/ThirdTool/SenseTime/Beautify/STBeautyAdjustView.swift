...
class STBeautyAdjustView: UIView {
    ...
    private lazy var topItemView: STBeautyTopItemView = {
        ...
        view.frame = CGRect.init(x: 0, y: 50, width: kLet_itemData, height: 50)
        view.delegate = self
        return view
    }()
    ...
    private lazy var itemCollectionView: STBeautyCollectionView = {
        let view = STBeautyCollectionView.init(frame: CGRect.init(x: 0, y: 50+18, width: kLet_itemData, height: beautyContainerView.height-50-18-kLet_messageTopText), collectionViewLayout: UICollectionViewLayout())
        return view
    }()
    
    ///  滤镜列表
    private lazy var filterListView: STFilterListView = {
        let view = STFilterListView.init(frame: CGRect.init(x: kLet_itemData, y: 50+18, width: kLet_itemData, height: beautyContainerView.height-50-18-kLet_messageTopText))
        return view
    }()
    
    
    private lazy var beautySlider: STBeautySlider = {
        let view = STBeautySlider.init(frame: CGRect.init(x: 22, y: 0, width: kLet_itemData-44, height: 40))
        view.minimumTrackTintColor = UIColor.viewColor()
        ...
        view.setThumbImage(UIImage.markTag(name: "st_slider_thumb"), for: .normal)
...
extension STBeautyAdjustView: STBeautyTopItemViewDelegate {
    ...
    func show() {
        ...
        popView = RecommendReactiveCompatible.init(frame: UIScreen.main.bounds)
    ...
    func changeTopItem(senderTag: Int) {
        ...
        if itemType == .Filter {
            ...
            self.beautyContainerView.setContentOffset(CGPoint.init(x: kLet_itemData, y: 0), animated: true)
...
extension STBeautyAdjustView {
    ...
    func setupSubviews() {
        ...
        beautyContainerView.frame = CGRect.init(x: 0, y: 50, width: kLet_itemData, height: self.height - 50)
