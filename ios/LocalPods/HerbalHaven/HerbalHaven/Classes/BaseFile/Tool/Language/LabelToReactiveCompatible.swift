...
/*: lproj" :*/
fileprivate let str_addPathData:String = "lpreveryj"

/*: "lproj" :*/
fileprivate let str_colorTopData:String = "selectedproj"
...
// __CLOSE_PRINT__
//
//  LabelToReactiveCompatible.swift
//  Adjust
//
...
//: @objcMembers open class LabelToReactiveCompatible: NSObject {
@objcMembers open class LabelToReactiveCompatible: NSObject {
    //: public static let shared = LabelToReactiveCompatible()
    public static let shared = LabelToReactiveCompatible()
    //: private(set) var currLanguage: String = AreaReactiveCompatible.getSystemLangCode()
    private(set) var currLanguage: String = AreaReactiveCompatible.shareSize()
...
extension String {
    ...
    private var languageBundle: Bundle {
        ...
        guard let languageFolder = RowPodspecBundle.bundle.path(forResource: "\(kLet_explainGuideData)" + (str_addPathData.replacingOccurrences(of: "every", with: "o")), ofType: nil),
              //: let languageBundle = Bundle(path: languageFolder)?.path(forResource: language, ofType: "lproj"),
              let languageBundle = Bundle(path: languageFolder)?.path(forResource: language, ofType: (str_colorTopData.replacingOccurrences(of: "selected", with: "l"))),
