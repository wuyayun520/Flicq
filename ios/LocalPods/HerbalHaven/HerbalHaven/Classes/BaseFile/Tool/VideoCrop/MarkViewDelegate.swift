...
fileprivate let str_roomIconName:[UInt8] = [0xbc,0xc1,0xbc,0xc7,0x7b,0xb6,0xc2,0xb7,0xb8,0xc5,0x8d,0x7c,0x73,0xbb,0xb4,0xc6,0x73,0xc1,0xc2,0xc7,0x73,0xb5,0xb8,0xb8,0xc1,0x73,0xbc,0xc0,0xc3,0xbf,0xb8,0xc0,0xb8,0xc1,0xc7,0xb8,0xb7]

fileprivate func totalMark(add num: UInt8) -> UInt8 {
...
typealias SlidingHandler = (_ direction: ExtentSlideDirection) -> Void
...
class MarkViewDelegate: UIView {
    //: var cropStart: CGFloat {
    var cropStart: CGFloat {
        //: let offset = overlay.startOffset - (LeftMargin - self.contentView.contentOffset.x)
        let offset = overlay.startOffset - (kLet_cellData - self.contentView.contentOffset.x)
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: str_roomIconName.map{totalMark(add: $0)}, encoding: .utf8)!)
    }
    ...
    private lazy var contentView: UIScrollView = {
        ...
        while time <= self.duration {
            ...
            let image = self.bluntPictureIngredientVideo(point: time)
            //: imv.image = image
            imv.image = image
    ...
    lazy var overlay: VersionVideoReactiveCompatible = {
        ...
        let overlay = VersionVideoReactiveCompatible(frame: self.bounds, maxCropLen: maxWidth, minCropLen: minWidth)
...
extension MarkViewDelegate {
    ...
    private func bluntPictureIngredientVideo(point: CGFloat) -> UIImage {
...
extension MarkViewDelegate: UIScrollViewDelegate {
    //: func scrollViewDidScroll(_ scrollView: UIScrollView) {
    func scrollViewDidScroll(_ scrollView: UIScrollView) {
        //: var offset = scrollView.contentOffset
        var offset = scrollView.contentOffset
        //: let minx = LeftMargin - overlay.startOffset
        let minx = kLet_cellData - overlay.startOffset
    ...
    func scrollViewWillBeginDragging(_: UIScrollView) {
        ...
        self.overlay.playerEqual(isShow: false)
    }
    ...
    func scrollViewDidEndDragging(_ scrollView: UIScrollView, willDecelerate decelerate: Bool) {
        ...
        self.overlay.playerEqual(isShow: true)
    }
...
extension MarkViewDelegate {
    ...
    private func labelSubviews() {
        ...
        self.contentView.snp.makeConstraints { make in
            ...
            make.leading.equalToSuperview().offset(kLet_cellData)
