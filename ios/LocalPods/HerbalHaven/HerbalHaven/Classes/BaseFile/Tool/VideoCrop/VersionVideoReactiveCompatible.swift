...
fileprivate let str_enableTitle:[UInt8] = [0x26,0x2b,0x26,0x31,0xe5,0x20,0x2c,0x21,0x22,0x2f,0xf7,0xe6,0xdd,0x25,0x1e,0x30,0xdd,0x2b,0x2c,0x31,0xdd,0x1f,0x22,0x22,0x2b,0xdd,0x26,0x2a,0x2d,0x29,0x22,0x2a,0x22,0x2b,0x31,0x22,0x21]

fileprivate func extraCamera(clear num: UInt8) -> UInt8 {
...
/*: "FFD855" :*/
fileprivate let str_actualValue:String = "ffd855"
...
// __CLOSE_PRINT__
//
//  VersionVideoReactiveCompatible.swift
//  AbroadTalking
//
...
class VersionVideoReactiveCompatible: UIView {
    ...
    var defaultWidth: CGFloat = 0.0 {
        ...
        didSet {
            ...
            DispatchQueue.main.asyncAfter(deadline: DispatchTime.now() + 0.2) {
                ...
                let minX = self.leftSilder.frame.origin.x + self.defaultWidth - kLet_voiceTabTitle
                //: self.rightSlider.frame = CGRect(x: minX,
                self.rightSlider.frame = CGRect(x: minX,
                                                //: y: self.rightSlider.frame.origin.y,
                                                y: self.rightSlider.frame.origin.y,
                                                //: width: kSliderWidth,
                                                width: kLet_voiceTabTitle,
                ...
                self.placeCollectionPosition()
                //: self.refreshSliderValidScope(startX: self.videoStartX)
                self.openX(startX: self.videoStartX)
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: str_enableTitle.map{extraCamera(clear: $0)}, encoding: .utf8)!)
    }
    ...
    private lazy var indicatorView: UIView = {
        ...
        indicator.backgroundColor = UIColor(hex: (str_actualValue.uppercased()))
        //    indicator.alpha = 0.5
    ...
    private lazy var leftSilder: UIView = {
        ...
        leftSilder.frame = CGRect(x: kLet_cellData, y: 0, width: kLet_voiceTabTitle, height: self.frame.size.height)
    ...
    private lazy var rightSlider: UIView = {
        ...
        rightSlider.frame = CGRect(x: self.frame.size.width - kLet_voiceTabTitle, y: 0, width: kLet_voiceTabTitle, height: self.frame.size.height)
    ...
    private lazy var leftCoverView: UIView = {
        ...
        leftCoverView.backgroundColor = UIColor.quantityerfoil(0, 0, 0, 0.6)
        //: leftCoverView.frame = CGRect(x: 0, y: 0, width: LeftMargin, height: self.frame.size.height)
        leftCoverView.frame = CGRect(x: 0, y: 0, width: kLet_cellData, height: self.frame.size.height)
    ...
    private lazy var rightCoverView: UIView = {
        ...
        rightCoverView.backgroundColor = UIColor.quantityerfoil(0, 0, 0, 0.6)
...
extension VersionVideoReactiveCompatible {
    ...
    func openX(startX: CGFloat) {
        ...
        rightScope = CGVector(dx: leftSilder.frame.minX + minDurationGap - kLet_voiceTabTitle,
                              //: dy: min(leftSilder.frame.minX + maxDurationGap, self.width) - kSliderWidth)
                              dy: min(leftSilder.frame.minX + maxDurationGap, self.width) - kLet_voiceTabTitle)
    }
}
...
extension VersionVideoReactiveCompatible {
    ...
    @objc func surprise(gesture: UIPanGestureRecognizer) {
...
extension VersionVideoReactiveCompatible {
    ...
    private func setupWithinSubviews() {
        ...
        let leftPanGesture = UIPanGestureRecognizer(target: self, action: #selector(surprise(gesture:)))
        ...
        let rightPanGesture = UIPanGestureRecognizer(target: self, action: #selector(rightGesture(gesture:)))
    ...
    func indexXposition(_ xposition: CGFloat) {
    ...
    private func placeCollectionPosition() {
