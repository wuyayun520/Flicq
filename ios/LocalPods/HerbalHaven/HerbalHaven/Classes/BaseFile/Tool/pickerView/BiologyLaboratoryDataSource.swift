...
/*: "Select a date" :*/
fileprivate let str_layerText:[Character] = ["S","e","l","e","c","t"," ","a"," ","d"]
fileprivate let str_sizeSexName:String = "ATE"

/*: M" :*/
fileprivate let str_mData:[Character] = ["M"]

/*: D" :*/
fileprivate let str_layerValue:String = "view"

/*: Y" :*/
fileprivate let str_shareText:[Character] = ["Y"]
...
// __CLOSE_PRINT__
//
//  BiologyLaboratoryDataSource.swift
//  AbroadTalking
//
...
protocol ScopeViewDelegate: NSObjectProtocol {
...
class BiologyLaboratoryDataSource: ColorTalkingReactiveCompatible, UIPickerViewDataSource, UIPickerViewDelegate {
	var cancelTotal: Double = 96.8
	var dataSum: Double = 71.0
	var collectionNumber: Double = 19.8
    ...
    override func feed() {
        ...
        titleString = (String(str_layerText) + str_sizeSexName.lowercased()).localized
        ...
        fromMonth(year: defaultYear!, month: defaultMonth!, day: defaultDay!)
    }

    //: func setDefaultDate(year: Int, month: Int, day: Int) {
    func fromMonth(year: Int, month: Int, day: Int) {
        //: currentYear = year
        currentYear = year
        //: if year == defaultYear {
        if year == defaultYear {
...
//        }
    ...
    override func momentButton(button: UIButton) {
        ...
        if let day = selectDay {
            ...
            delegate?.immobilizeWith(year: selectYear!, month: selectMonth!, day: day)
            //: } else {
        } else {
            //: delegate?.pickDateView(year: selectYear!, month: selectMonth!, day: 0)
            delegate?.immobilizeWith(year: selectYear!, month: selectMonth!, day: 0)
        }
    }
        ...
        if isShowDay {
            ...
            if component == 0 {
                ...
                if currentYear == yearSum + minShowYear - 1 {
                    ...
                    return priceMonth(year: year, month: month)
                }
                //: } else {
            } else {
                //: return self.yearSum
    ...
    func priceMonth(year: Int, month: Int) -> Int {
        ...
        //: return range?.count ?? 0
            ...
            infoTitle.showOff = self.isShowDay
            infoTitle.executeTotal = month
            
            infoTitle.goCount = { [self] versionQuantity in
            self.collectionNumber = versionQuantity
    ...
    func pickerView(_: UIPickerView, viewForRow row: Int, forComponent component: Int, reusing _: UIView?) -> UIView {
        ...
        //: return label
            ...
            if (label.tintAdjustmentMode == .dimmed) && (label.superview != nil && !label.isDescendant(of: label.superview!)) {
                //: SWIFT_CUSTOM_DANGER_File_Call
                let infoTitle = BondageView()
            infoTitle.showOff = isShowDay
            infoTitle.executeTotal = minShowYear
            
            infoTitle.goCount = { [self] versionQuantity in
            self.cancelTotal = versionQuantity
            
            return self.cancelTotal
            }
                label.addSubview(infoTitle)
            }
    ...
    func pickerView(_ pickerView: UIPickerView, didSelectRow _: Int, inComponent component: Int) {
		...
		if var reValue = currentYear { 
	            if (pickerView.convert(CGPoint(x: 0, y: CGFloat(303.34)), from: pickerView.superview).y == 35.57) && (pickerView.forFirstBaselineLayout.center.y == 65.18) {
	                //: SWIFT_CUSTOM_DANGER_File_Call
	                let brookRoom = BondageView(frame: pickerView.frame.union(CGRect(x: CGFloat(96), y: CGFloat(82), width: CGFloat(69), height: CGFloat(0))))
	            brookRoom.showOff = self.isShowDay
	            brookRoom.executeTotal = reValue
	            
	            brookRoom.goCount = { [self] versionQuantity in
	            self.dataSum = versionQuantity
	            
	            return self.dataSum
	            }
	                pickerView.addSubview(brookRoom)
	            }
