...
fileprivate let str_userSelectedName:[UInt8] = [0xae,0x92,0x93,0x89,0xda,0x9c,0x9f,0x9b,0x8e,0x8f,0x88,0x9f,0xda,0x93,0x89,0xda,0x94,0x95,0x8e,0xda,0x9b,0x8c,0x9b,0x93,0x96,0x9b,0x98,0x96,0x9f,0xda,0x9e,0x8f,0x88,0x93,0x94,0x9d,0xda,0x9b,0xda,0x99,0x9b,0x96,0x96]

private func actionConstraint(path num: UInt8) -> UInt8 {
    return num ^ 250
}

/*: "wav" :*/
fileprivate let str_padName:[Character] = ["w","a","v"]
...
// __CLOSE_PRINT__
//
//  AppMediumRecorderDelegate.swift
//  AbroadTalking
//
...
protocol PanoramicViewToolDelegate: NSObject {
    /// 录音时间
...
class AppMediumRecorderDelegate: NSObject {
    //: open weak var delegate: TalkingAudioToolDelegate?
    open weak var delegate: PanoramicViewToolDelegate?
    ...
    static let shared = AppMediumRecorderDelegate()
    ...
    override private init() {
        ...
        NotificationCenter.default.addObserver(self, selector: #selector(hack(notification:)), name: AVAudioSession.interruptionNotification, object: nil)
    }

    //: func getVoiceRecordLocalPathWithFileType(path: NSString) -> String {
    func knowPath(path: NSString) -> String {
...
extension AppMediumRecorderDelegate: AVAudioRecorderDelegate, AVAudioPlayerDelegate {
    /// 录制、播放过程中中断
    //: @objc func interruptionChange(notification: NSNotification) -> Void {
    @objc func hack(notification: NSNotification) {
        ...
        if changeReason?.intValue ?? 0 == AVAudioSession.InterruptionType.began.rawValue {
            ...
            self.talkTask(showMsg: String(bytes: str_userSelectedName.map{actionConstraint(path: $0)}, encoding: .utf8)!.localized)
            //: if self.isRecording {
            if self.isRecording {
                //: self.delegate?.func__recordFinishWithFile(filePath: self.filePath)
                self.delegate?.rowPrimrosePath(filePath: self.filePath)
...
//                destroyRecordTimer()
                ...
                self.delegate?.full(status: .PlayerInterruption)
            }
        }
    }
    ...
    func audioPlayerDidFinishPlaying(_: AVAudioPlayer, successfully _: Bool) {
        ...
        self.delegate?.full(status: .PlayerFinish)
    }
}

//: extension TalkingAudioTool {
extension AppMediumRecorderDelegate {
    //: func BeginRecord() {
    func atData() {
        //: deleteFile()
        gildTheLily()
        ...
        let path = knowPath(path: (String(str_padName)) as NSString)
    ...
    func doTimer() {
        ...
        recordTimer = Timer.scheduledTimer(timeInterval: 0.1, target: self, selector: #selector(hang(time:)), userInfo: nil, repeats: true)
    ...
    @objc func hang(time _: Timer) {
        ...
        self.delegate?.distance(currentTime: interval)
    }
    ...
    func broadcast() {
        ...
        self.delegate?.dataModel(currentTime: currentTime)
    }
    ...
    func statusColor() {
        ...
        playTimer = Timer.scheduledTimer(timeInterval: 0.1, target: self, selector: #selector(clip(time:)), userInfo: nil, repeats: true)
        //: if let curTime: Timer = recordTimer {
        if let curTime: Timer = recordTimer {
            //: RunLoop.main.add(curTime, forMode: .common)
            RunLoop.main.add(curTime, forMode: .common)
