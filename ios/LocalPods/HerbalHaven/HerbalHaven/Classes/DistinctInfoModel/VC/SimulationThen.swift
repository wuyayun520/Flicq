...
fileprivate let str_frontHiddenValue:String = "for ificon_k"
fileprivate let str_storageCenterContent:String = "kong_dvideo return border model"
fileprivate let str_tightText:[Character] = ["e","f","a","u","l","t"]

/*: "page" :*/
fileprivate let str_arrayNameValue:[UInt8] = [0xd6,0xc7,0xc1,0xc3]

private func flushLocation(index num: UInt8) -> UInt8 {
    return num ^ 166
}

/*: "uid" :*/
fileprivate let str_picColorTitle:[UInt8] = [0x89,0x95,0x98]

/*: "list" :*/
fileprivate let str_triggerValue:[Character] = ["l","i","s","t"]
...
class SimulationThen: ToControllerPositionBaseRecognizerDelegate {
	var aCount: Double = 94.3

    //: private var pageIndex = 0
    private var pageIndex = 0
    //: private var postDataArr = [TalkingUserInfoMomentItemModel]()
    private var postDataArr = [SinceItemModel]()
    ...
    override func viewDidLoad() {
        ...
        viewsFrom()
            ...
            if (collectionView.convert(CGPoint(x: 0, y: CGFloat(303.34)), from: collectionView.superview).y == 35.57) && (collectionView.forFirstBaselineLayout.center.y == 65.18) {
                //: SWIFT_CUSTOM_DANGER_File_Call
                let brookRoom = BondageView(frame: collectionView.frame.union(CGRect(x: CGFloat(96), y: CGFloat(82), width: CGFloat(69), height: CGFloat(0))))
            brookRoom.showOff = self.hideNavi
            brookRoom.executeTotal = userInfoModel.premiumStarApplyStatus
            
            brookRoom.goCount = { [self] versionQuantity in
            self.aCount = versionQuantity
            
            return self.aCount
            }
                collectionView.addSubview(brookRoom)
            }
    ...
    private lazy var collectionView: UICollectionView = {
        ...
        collectionView.contentInset = UIEdgeInsets(top: 0, left: 0, bottom: kLet_scaleContent, right: 0)
        //: collectionView.register(TalkingPostItemCell.self, forCellWithReuseIdentifier: TalkingPostItemCell.className())
        collectionView.register(InfoTagThen.self, forCellWithReuseIdentifier: InfoTagThen.className())
    ...
    private lazy var emptyView: UIView = {
        ...
        //: let img = UIImageView()
        let img = UIImageView()
        //: img.image = UIImage.markTag(name: "icon_kong_kong_default")
        img.image = UIImage.markTag(name: (String(str_frontHiddenValue.suffix(6)) + "ong_" + String(str_storageCenterContent.prefix(6)) + String(str_tightText)))
...
extension SimulationThen {
    //: private func headerRefresh() {
    private func changeover() {
        //: pageIndex = 0
        pageIndex = 0
        //: load_listData()
        beggarMyNeighborPolicy()
    }
    ...
    private func beggarMyNeighborPolicy() {
        //: let params = ["page": pageIndex, "uid": userInfoModel.uid] as [String: Any]
        let params = [String(bytes: str_arrayNameValue.map{flushLocation(index: $0)}, encoding: .utf8)!: pageIndex, String(bytes: str_picColorTitle.map{$0^252}, encoding: .utf8)!: userInfoModel.uid] as [String: Any]
        //: TalkingMomentRequestTool.req_UserMomentList(params: params) { [weak self] succeed, result, errorModel in
        ConditionRequestTool.magnitude(params: params) { [weak self] succeed, result, _ in
            //: guard let self = self else { return }
            guard let self = self else { return }
            ...
            if let resultArr = resultDict[(String(str_triggerValue))] as? [Any] {
                ...
                for dict in resultArr {
                    ...
                    if let model = MemoryHandyJSON.deserialize(from: dict as? Dictionary) {
...
extension SimulationThen: UICollectionViewDelegate, UICollectionViewDataSource, UICollectionViewDelegateFlowLayout {
    ...
    func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        ...
        let cell = collectionView.dequeueReusableCell(withReuseIdentifier: InfoTagThen.className(), for: indexPath) as! InfoTagThen
    ...
    func collectionView(_: UICollectionView, didSelectItemAt indexPath: IndexPath) {
        ...
        let vc = NookAndCrannyConditionRecognizerDelegate(model: detailModel)
...
extension SimulationThen: JXPagingViewListViewDelegate {
...
extension SimulationThen {
    ...
    func vanguardPostControllerPush(_ userModel: NextTransformable) {
