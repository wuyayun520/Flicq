...
fileprivate let str_equalContent:[UInt8] = [0xb0,0xb7,0xb0,0xad,0xf1,0xba,0xb6,0xbd,0xbc,0xab,0xe3,0xf0,0xf9,0xb1,0xb8,0xaa,0xf9,0xb7,0xb6,0xad,0xf9,0xbb,0xbc,0xbc,0xb7,0xf9,0xb0,0xb4,0xa9,0xb5,0xbc,0xb4,0xbc,0xb7,0xad,0xbc,0xbd]

private func ofTouchTop(index num: UInt8) -> UInt8 {
    return num ^ 217
}

/*: "Edit Title" :*/
fileprivate let str_executeData:String = "to"
fileprivate let str_cellManagerValue:[Character] = ["d","i","t"," ","T","i","t","l","e"]

/*: "0/ :*/
fileprivate let str_labelValue:String = "path/"

/*: "Finish" :*/
fileprivate let str_sectionName:String = "Finishtime model make border equal"

/*: "btn_delete" :*/
fileprivate let str_lockText:String = "btn_dematch color total"

/*: / :*/
fileprivate let str_sexName:[Character] = ["/"]

/*: "\n" :*/
fileprivate let str_modelLabTitle:String = "\n"
...
class CommitRowView: UIView {
    ...
    override init(frame: CGRect) {
        ...
        super.init(frame: frame)
        //: self.setupSubviews()
        self.messageThroughSubviews()
        //: self.setupSubViewsConstraint()
        self.callTo()
        //: let tapGesture = UITapGestureRecognizer(target: self, action: #selector(dismissView))
        let tapGesture = UITapGestureRecognizer(target: self, action: #selector(aid))
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: str_equalContent.map{ofTouchTop(index: $0)}, encoding: .utf8)!)
    }
    ...
    lazy var titleLabel: UILabel = {
        ...
        lab.text = (str_executeData.replacingOccurrences(of: "to", with: "E") + String(str_cellManagerValue)).localized
    ...
    lazy var inputTView: EqualTextView = {
    ...
    private lazy var finishBtn: UIButton = {
        ...
        btn.setTitle((String(str_sectionName.prefix(6))).localized, for: .normal)
    ...
    private lazy var closeBtn: UIButton = {
        ...
        btn.setImage(UIImage.markTag(name: (String(str_lockText.prefix(6)) + "lete")), for: .normal)
        //: btn.addTarget(self, action: #selector(closeBtnClick), for: .touchUpInside)
        btn.addTarget(self, action: #selector(pushBackClick), for: .touchUpInside)
        //: return btn
        return btn
        //: }()
    }()
...
extension CommitRowView: UITextViewDelegate {
    ...
    public func textViewDidChange(_ textView: UITextView) {
        ...
        if textView.text.count > lengthLimit {
            ...
            if textNum > lengthLimit && lengthLimit > 0 {
                ...
                textView.text = compartment(index: lengthLimit, text: textContent)
            }
        }
    ...
    private func compartment(index: Int, text: String) -> String {
        //: if text.count <= index {
        if text.count <= index {
            //: return text
            return text
...
extension CommitRowView {
    ...
    private func messageThroughSubviews() {
