...
/*: "id" :*/
fileprivate let str_availablePinData:[Character] = ["i","d"]
...
struct MemoryHandyJSON: HandyJSON {
    //: var uid: String?
    var uid: String?
    //: var sex: String?
    var sex: String?
    ...
    var pic = [BackgroundHandyJSON]()
    //: var momentType: String?
    var momentType: String?
    //: var videoInfo: TalkingMomentVideoModel?
    var videoInfo: ResolutionTransformable?
...
enum ArrayColumnConvertible: String {
    //: case nones  = "0"
    case nones = "0"
    //: case Photo = "1"
    case Photo = "1"
...
struct ResolutionTransformable: HandyJSON {
    //: var videoUrl = ""
    var videoUrl = ""
    //: var imgUrl = ""
    var imgUrl = ""
    ...
    var status: RecordContentConvertible?
}

///
//: enum MomentVideoStatus: String {
enum RecordContentConvertible: String {
    //: case NoPass = "-1"   // 未通过
    case NoPass = "-1" // 未通过
    //: case Audit = "0"     // 审核中
    case Audit = "0" // 审核中
...
extension MemoryHandyJSON {
    //: mutating func caculateItemHeight(isPost: Bool = false) {
    mutating func barEnd(isPost: Bool = false) {
        ...
        if content!.count > 0 {
            ...
            //: textHeight = Int(TalkingMomentTextSizeView.caculateSizeWithText(text: content!, size: CGSize.init(width: ScreenWidth-54, height: CGFloat(MAXFLOAT)), font: UIFont.biologyLaboratoryEqual(type: .Regular, fontSize: 17), lineNumber: lineNumber).height + (lineNumber==0 ? 20:0))
            textHeight = Int(PointSizeView.noRoot(text: content!, size: CGSize(width: kLet_itemData - 54, height: CGFloat(MAXFLOAT)), font: UIFont.biologyLaboratoryEqual(type: .Regular, fontSize: 17), lineNumber: lineNumber).height + (lineNumber == 0 ? 20 : 0))
        }

        //: if momentType == MomentType.Photo.rawValue || momentType == MomentType.Live.rawValue {
        if momentType == ArrayColumnConvertible.Photo.rawValue || momentType == ArrayColumnConvertible.Live.rawValue {
            //: if pic.count==1 {
            if pic.count == 1 {
                //: photesHeight = 212
                photesHeight = 212
