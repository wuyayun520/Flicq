...
/*: "%@%@" :*/
fileprivate let str_millValue:String = "%@%@"

/*: "/TXCache" :*/
fileprivate let str_backSizeName:String = "/TXCacheelse biology"
...
protocol FromReactiveCompatible: NSObject {
    //: func func__playerStatus(player: TalkingVideoPlayerManager, status: VideoPlayerStatus)
    func alertControl(player: NameTextThen, status: MenuPlayerStatus)

    //: func func__playerDuration(player: TalkingVideoPlayerManager, duration: Int, currentTime: Int)
    func cuttingEdge(player: NameTextThen, duration: Int, currentTime: Int)

    //: func func__playerProgress(player: TalkingVideoPlayerManager, progress: CGFloat)
    func phone(player: NameTextThen, progress: CGFloat)
}

//: class TalkingVideoPlayerManager: NSObject {
class NameTextThen: NSObject {
    //: var isPlaying = false
    var isPlaying = false
    //: var isMute = false
    var isMute = false
    ...
    static let shared = NameTextThen()
    ...
    lazy var player: TXVodPlayer = {
        ...
        let cachePath = NSString(format: "%@%@", NSSearchPathForDirectoriesInDomains(.documentDirectory, .userDomainMask, true).first!, (String(str_backSizeName.prefix(8)))) as String
    ...
    lazy var timer: Timer = {
        ...
        let timer = Timer.scheduledTimer(timeInterval: 0.25, target: self, selector: #selector(romp), userInfo: nil, repeats: true)
...
extension NameTextThen {
    //: func playerWithUrlAndVideoView(url: String, view: UIView) {
    func betweenWith(url: String, view: UIView) {
        //: let newUrl = url.replacingOccurrences(of: UrlDomin.urlAes256Decrypt(), with: ReplaceUrlDomain)
        let newUrl = url.replacingOccurrences(of: UrlDomin.urlAes256Decrypt(), with: kLet_keyViewName)
    ...
    @objc func romp() {
        ...
        if duration() > 0 {
            ...
            self.delegate?.phone(player: self, progress: CGFloat(currentTime() / duration()))
        }
    }

    //: func setRenderMode(renderMode: EnumTypeRenderMode) {
    func goopMode(renderMode: BodyRenderMode) {
    ...
    func example(time: Float) {
        //: self.player.seek(time)
        self.player.seek(time)
    }

    //: func height() ->Int {
    func overloadHeight() -> Int {
    ...
    func element(bEnable: Bool) {
        //: var isMute = bEnable
        var isMute = bEnable
        //: if TalkingSocketManager.shared.isTalking == true {
        if ManagingDirectorReactiveCompatible.shared.isTalking == true { // 音视频通话时静音
            //: isMute = true
            isMute = true
        }
    ...
    func dismissRow() {
        ...
        if self.playStatus != .Pausing {
            ...
            self.delegate?.alertControl(player: self, status: self.playStatus!)
        }
    }

    //: func isPlayings() -> Bool {
    func colorVideo() -> Bool {
...
extension NameTextThen: TXVodPlayListener {
    ...
    func onPlayEvent(_ player: TXVodPlayer!, event EvtID: Int32, withParam _: [AnyHashable: Any]!) {
        ...
        case PLAY_EVT_PLAY_PROGRESS.rawValue:
            ...
            self.delegate?.cuttingEdge(player: self, duration: Int(duration), currentTime: Int(currentTime))
