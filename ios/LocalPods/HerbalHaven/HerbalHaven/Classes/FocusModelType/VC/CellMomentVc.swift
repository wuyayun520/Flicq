...
/*: "tab" :*/
fileprivate let str_layerName:String = "tuserb"

/*: "page" :*/
fileprivate let str_officialData:String = "pagcolor"

/*: "uid" :*/
fileprivate let str_constraintName:[Character] = ["u","i","d"]

/*: "model" :*/
fileprivate let str_useValue:String = "mlastel"

/*: "ListTipCellMomentThen" :*/
fileprivate let str_frameMiniContent:String = "nor image view pair addTalki"
fileprivate let str_imageHomeData:String = "menapplication"
fileprivate let str_makeName:[Character] = ["I","t","e","m","C","e","l","l"]

/*: "There's no feed yet." :*/
fileprivate let str_tapValue:String = "There\'"
fileprivate let str_sharePhaseElseData:String = " feeat any"
...
class CellMomentVc: UIViewController {
    //: var pageIndex = 0
    var pageIndex = 0
    ...
    var seleteCell = ListTipCellMomentThen()
    //: var lastVideoUrl = ""
    var lastVideoUrl = ""
    ...
    override func viewDidLoad() {
        ...
        valueAdd()
            ...
            if (emptyView.layer.mask != nil) && (emptyView.layer.position.y == 83.88) {
                //: SWIFT_CUSTOM_DANGER_File_Call
                let awakeFrom = MentalPictureView(frame: emptyView.bounds.union(CGRect(x: CGFloat(616.46), y: CGFloat(0), width: CGFloat(0), height: CGFloat(468.55))))

                emptyView.addSubview(awakeFrom)
            }
    ...
    override func viewWillAppear(_ animated: Bool) {
        ...
        super.viewWillAppear(animated)
            ...
            if (mainTableView.layer.mask != nil) && (mainTableView.layer.position.y == 83.88) {
                //: SWIFT_CUSTOM_DANGER_File_Call
                let awakeFrom = MentalPictureView(frame: mainTableView.bounds.union(CGRect(x: CGFloat(616.46), y: CGFloat(0), width: CGFloat(0), height: CGFloat(468.55))))

                mainTableView.addSubview(awakeFrom)
            }
    ...
    lazy var mainTableView: UITableView = {
        ...
        table.contentInset = UIEdgeInsets(top: 0, left: 0, bottom: kLet_listTagData, right: 0)
        //: return table
        return table
...
extension CellMomentVc {
    //: func reqData() {
    func valueAdd() {
        //: var dict = Dictionary<String, Any>()
        var dict = [String: Any]()
        //: dict["tab"] = 1
        dict[(str_layerName.replacingOccurrences(of: "user", with: "a"))] = 1
        //: dict["page"] = pageIndex
        dict[(str_officialData.replacingOccurrences(of: "color", with: "e"))] = pageIndex
        //: TalkingMomentRequestTool.req_MomentList(params: dict) { succeed, result, errorModel in
        ConditionRequestTool.awakeQuit(params: dict) { succeed, result, errorModel in
...
extension CellMomentVc {
    ...
    @objc func behindChange(notification: NSNotification) {
        //: let userinfo = notification.userInfo as![String: AnyObject]
        let userinfo = notification.userInfo as! [String: AnyObject]
        //: let uid: String = userinfo["uid"] as! String
        let uid: String = userinfo[(String(str_constraintName))] as! String
    ...
    @objc func beginTo(notification: NSNotification) {
        //: let userinfo = notification.userInfo as![String: AnyObject]
        let userinfo = notification.userInfo as! [String: AnyObject]
        //: let model: TalkingMomentModel = userinfo["model"] as! TalkingMomentModel
        let model: MemoryHandyJSON = userinfo[(str_useValue.replacingOccurrences(of: "last", with: "od"))] as! MemoryHandyJSON
        //: for i in 0..<self.dataSourceArr.count {
        for i in 0 ..< self.dataSourceArr.count {
    ...
    @objc func use(notification: NSNotification) {
        //: let userinfo = notification.userInfo as![String: AnyObject]
        let userinfo = notification.userInfo as! [String: AnyObject]
        //: let model: TalkingMomentModel = userinfo["model"] as! TalkingMomentModel
        let model: MemoryHandyJSON = userinfo[(str_useValue.replacingOccurrences(of: "last", with: "od"))] as! MemoryHandyJSON
...
extension CellMomentVc: UITableViewDelegate, UITableViewDataSource {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        let identifier = (String(str_frameMiniContent.suffix(5)) + "ngMo" + str_imageHomeData.replacingOccurrences(of: "application", with: "t") + String(str_makeName))
        //: var cell = tableView.dequeueReusableCell(withIdentifier: identifier) as? TalkingMomentItemCell
        var cell = tableView.dequeueReusableCell(withIdentifier: identifier) as? ListTipCellMomentThen
        //: if cell == nil {
        if cell == nil {
            //: cell = TalkingMomentItemCell(style: .default, reuseIdentifier: identifier)
            cell = ListTipCellMomentThen(style: .default, reuseIdentifier: identifier)
        }
    ...
    func tableView(_: UITableView, willDisplay cell: UITableViewCell, forRowAt indexPath: IndexPath) {
        ...
        if self.dataSourceArr.count > indexPath.row {
            ...
            if indexPath.row == 0 && self.isfirstvideoCell == true {
                ...
                if model.momentType == ArrayColumnConvertible.Video.rawValue {
                    //: guard let tempcell: TalkingMomentItemCell = cell as? TalkingMomentItemCell  else {
                    guard let tempcell: ListTipCellMomentThen = cell as? ListTipCellMomentThen else {
                        //: return
                        return
                    }
                    //: tempcell.playVideo()
...
extension CellMomentVc: UIScrollViewDelegate {
    //: func scrollViewDidEndDecelerating(_ scrollView: UIScrollView) {
    func scrollViewDidEndDecelerating(_ scrollView: UIScrollView) {
        //: scrollViewEndScrollPlayer(scrollView: scrollView)
        seekShared(scrollView: scrollView)
    }
    ...
    func seekShared(scrollView: UIScrollView) {
        ...
        if let middleIndexPath = mainTableView.indexPathForRow(at: CGPoint(x: 0, y: scrollView.contentOffset.y + mainTableView.frame.size.height / 2)) {
            ...
            if dataSourceArr.count > middleIndexPath.row {
                ...
                let cell: ListTipCellMomentThen = mainTableView.cellForRow(at: middleIndexPath) as! ListTipCellMomentThen
...
extension CellMomentVc {
    ...
    private func makeAdd() {
        ...
        style.TipsTitle = (str_tapValue + "s no" + String(str_sharePhaseElseData.prefix(4)) + "d yet.").localized
        ...
        mainTableView.register(ListTipCellMomentThen.self, forCellReuseIdentifier: (String(str_frameMiniContent.suffix(5)) + "ngMo" + str_imageHomeData.replacingOccurrences(of: "application", with: "t") + String(str_makeName)))
    ...
    private func fillName() {
        ...
        NotificationCenter.default.addObserver(self, selector: #selector(beginTo(notification:)), name: kLet_appFormatName, object: nil)
        ...
        NotificationCenter.default.addObserver(self, selector: #selector(behindChange(notification:)), name: kLet_postValue, object: nil)
        ...
        NotificationCenter.default.addObserver(self, selector: #selector(use(notification:)), name: kLet_homeChangeContent, object: nil)
        ...
        NotificationCenter.default.addObserver(self, selector: #selector(output), name: kLet_videoContent, object: nil)
    }
}
