...
fileprivate let str_viewMakeValue:[UInt8] = [0x7a,0x7f,0x7a,0x85,0x39,0x74,0x80,0x75,0x76,0x83,0x4b,0x3a,0x31,0x79,0x72,0x84,0x31,0x7f,0x80,0x85,0x31,0x73,0x76,0x76,0x7f,0x31,0x7a,0x7e,0x81,0x7d,0x76,0x7e,0x76,0x7f,0x85,0x76,0x75]

fileprivate func ifSelf(raw num: UInt8) -> UInt8 {
...
class RecordPlayerSlider: UIView {
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: str_viewMakeValue.map{ifSelf(raw: $0)}, encoding: .utf8)!)
    }
    ...
    private lazy var progressView: JointResolutionProgressView = {
        ...
        let v = JointResolutionProgressView(frame: self.frame, progressBgColor: self.progressBgColor, progressPlayedColor: self.progressPlayedColor, progressCachedColor: self.progressCachedColor)
    ...
    private lazy var panGesture: UIPanGestureRecognizer = {
        ...
        let v = UIPanGestureRecognizer(target: self, action: #selector(actionVersion(gesture:)))
        //: return v
        return v
        //: }()
    }()
    ...
    private lazy var tapGesture: UIPanGestureRecognizer = {
        ...
        let v = UIPanGestureRecognizer(target: self, action: #selector(colorEmpty(gesture:)))
        //: return v
        return v
        //: }()
    }()
...
extension RecordPlayerSlider {
    //: @objc func panGesture(gesture: UIPanGestureRecognizer) {
    @objc func actionVersion(gesture: UIPanGestureRecognizer) {
        // 移动的距离
    ...
    @objc func colorEmpty(gesture: UIPanGestureRecognizer) {
