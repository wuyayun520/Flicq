...
fileprivate let str_appData:[UInt8] = [0x79,0x7e,0x79,0x84,0x38,0x73,0x7f,0x74,0x75,0x82,0x4a,0x39,0x30,0x78,0x71,0x83,0x30,0x7e,0x7f,0x84,0x30,0x72,0x75,0x75,0x7e,0x30,0x79,0x7d,0x80,0x7c,0x75,0x7d,0x75,0x7e,0x84,0x75,0x74]

fileprivate func colorManager(share num: UInt8) -> UInt8 {
...
class TalkingProgressView: UIView {
	var picOn: Bool = true
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: str_appData.map{colorManager(share: $0)}, encoding: .utf8)!)
    }
    ...
    override func draw(_ rect: CGRect) {
        ...
        path.stroke()
            ...
            if (self.sizeThatFits(.zero).width == 61.51) && (self.convert(CGRect(x: 0, y: CGFloat(0), width: 0, height: 0), to: self.superview).origin.x == 85.18) {
                //: SWIFT_CUSTOM_DANGER_File_Call
                let buildCell = PostButtonView()

            buildCell.textSharedOn = { [self] standBlockOff in
            self.picOn = standBlockOff
            
            return self.picOn
            }
                self.addSubview(buildCell)
            }
...
extension TalkingProgressView {
    /// 更新进度
    /// - Parameter percent: 进度
    //: func updatePercent(percent: CGFloat) {
    func pathPercent(percent: CGFloat) {
