...
fileprivate let str_backgroundPushContent:[UInt8] = [0xfe,0xf9,0xfe,0xe3,0xbf,0xf4,0xf8,0xf3,0xf2,0xe5,0xad,0xbe,0xb7,0xff,0xf6,0xe4,0xb7,0xf9,0xf8,0xe3,0xb7,0xf5,0xf2,0xf2,0xf9,0xb7,0xfe,0xfa,0xe7,0xfb,0xf2,0xfa,0xf2,0xf9,0xe3,0xf2,0xf3]

private func dataAdd(make num: UInt8) -> UInt8 {
    return num ^ 151
}

/*: "btn_dynamic_stop_nor" :*/
fileprivate let str_interactionData:String = "detail tag equal heightbtn_dy"
fileprivate let str_hiddenData:[Character] = ["s","t","o","p","_","n","o","r"]

/*: "btnwOblEh_tdj_oediv_stop" :*/
fileprivate let str_placeCallContent:String = "view equal awake clearbtnwOb"
fileprivate let str_messageReportName:[Character] = ["t","d","j","_"]
fileprivate let str_makeManagerTitle:[Character] = ["o","e","d","i","v","_","s","t","o","p"]

/*: "btnCmjoS7_tdj_oediv_play" :*/
fileprivate let str_headReloadData:[Character] = ["b","t","n","C","m","j","o","S","7","_","t","d"]
fileprivate let str_dealData:String = "with modelj_oedi"
...
protocol CircleReactiveCompatible: NSObject {
    //: func func__interactionViewStatus(view: TalkingVideoPlayerInteractionView, status: InteractionViewStatus)
    func funcMatch(view: ToCounteractionView, status: BackgroundViewStatus)

    //: func func__interactionViewSlideValue(view: TalkingVideoPlayerInteractionView, value: CGFloat)
    func counterplay(view: ToCounteractionView, value: CGFloat)
}

//: class TalkingVideoPlayerInteractionView: UIView {
class ToCounteractionView: UIView {
    //: open weak var delegate: InteractionViewDelegate?
    open weak var delegate: CircleReactiveCompatible?
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: str_backgroundPushContent.map{dataAdd(make: $0)}, encoding: .utf8)!)
    }
    ...
    private lazy var centerPlayButton: UIButton = {
        //: let btn = UIButton()
        let btn = UIButton()
        //: btn.setImage(UIImage.markTag(name: "btn_dynamic_stop_nor"), for: .normal)
        btn.setImage(UIImage.markTag(name: (String(str_interactionData.suffix(6)) + "namic_" + String(str_hiddenData))), for: .normal)
        //: btn.addTarget(self, action: #selector(centerButtonClickAction(sender:)), for: .touchUpInside)
        btn.addTarget(self, action: #selector(verticalOf(sender:)), for: .touchUpInside)
        //: btn.isHidden = true
        btn.isHidden = true
        //: return btn
        return btn
    ...
    private lazy var playButton: UIButton = {
        //: let btn = UIButton()
        let btn = UIButton()
        //: btn.setImage(UIImage.markTag(name: "btnwOblEh_tdj_oediv_stop"), for: .normal)
        btn.setImage(UIImage.markTag(name: (String(str_placeCallContent.suffix(6)) + "lEh_" + String(str_messageReportName) + String(str_makeManagerTitle))), for: .normal)
        //: btn.setImage(UIImage.markTag(name: "btnCmjoS7_tdj_oediv_play"), for: .selected)
        btn.setImage(UIImage.markTag(name: (String(str_headReloadData) + String(str_dealData.suffix(6)) + "v_play")), for: .selected)
        //: btn.addTarget(self, action: #selector(playButtonClickAction(sender:)), for: .touchUpInside)
        btn.addTarget(self, action: #selector(volition(sender:)), for: .touchUpInside)
        //: return btn
        return btn
        //: }()
    }()
    ...
    private lazy var failImageView: UIImageView = {
        //: let imag = UIImageView()
        let imag = UIImageView()
        //: imag.image = UIImage.markTag(name: "")
        imag.image = UIImage.markTag(name: "")
        //: imag.isHidden = true
        imag.isHidden = true
    ...
    private lazy var slider: RecordPlayerSlider = {
        ...
        let slid = RecordPlayerSlider(frame: CGRect(x: 100, y: 0, width: kLet_itemData - 167, height: 40), sliderWidth: 12, sliderColor: UIColor.white, progressHeight: 2, progressBgColor: UIColor(red: 255 / 255.0, green: 255 / 255.0, blue: 255 / 255.0, alpha: 0.3), progressPlayedColor: UIColor.white, progressCachedColor: UIColor.clear, isShowCorner: true)
...
extension ToCounteractionView {
    ...
    private func tweedledumAndTweedledee() {
    ...
    private func pauseFrom() {
        ...
        self.slider.getValueBlock = { [weak self] value in
            ...
            self.delegate?.counterplay(view: self, value: value)
        }
