...
fileprivate let str_timeText:[UInt8] = [0x8a,0x8f,0x8a,0x95,0x49,0x84,0x90,0x85,0x86,0x93,0x5b,0x4a,0x41,0x89,0x82,0x94,0x41,0x8f,0x90,0x95,0x41,0x83,0x86,0x86,0x8f,0x41,0x8a,0x8e,0x91,0x8d,0x86,0x8e,0x86,0x8f,0x95,0x86,0x85]

fileprivate func rangeReturn(than num: UInt8) -> UInt8 {
...
/*: "gift_bg_dm" :*/
fileprivate let str_nameViewValue:[Character] = ["g","i","f","t","_","b","g","_","d","m"]

/*: "#FFF4B5" :*/
fileprivate let str_turnValue:[Character] = ["#","F","F","F","4","B","5"]

/*: "won " :*/
fileprivate let str_pageName:String = "won need view true array break"

/*: " coins in " :*/
fileprivate let str_replyName:String = " cframe model self string"
fileprivate let str_blockContent:String = "oiactions"

/*: " x :*/
fileprivate let str_succeedValue:[Character] = [" ","x"]

/*: "just sent" :*/
fileprivate let str_playAppValue:[Character] = ["j","u","s","t"," ","s","e","n","t"]

/*: "  :*/
fileprivate let str_plusColorValue:String = "view"

/*: "#4120C1" :*/
fileprivate let str_layerData:String = "#4120C1enable view view player"
...
class MemoryFloatView: UIView {
    /// 礼物model
    //: private var giftModel = SocialBroadcastModel()
    private var giftModel = MarkHandyJSON()
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: str_timeText.map{rangeReturn(than: $0)}, encoding: .utf8)!)
    }
    ...
    private lazy var bgView: UIImageView = {
        //: let v = UIImageView()
        let v = UIImageView()
        //: v.image = UIImage.markTag(name: "gift_bg_dm")
        v.image = UIImage.markTag(name: (String(str_nameViewValue)))
    ...
    private lazy var icon: UIImageView = {
        ...
        v.layer.borderColor = UIColor(hex: (String(str_turnValue)))?.cgColor
    ...
    private lazy var nicknameLab: UILabel = {
        ...
        lab.textColor = UIColor(hex: (String(str_turnValue)))
...
extension MemoryFloatView {
    /// 刷新视图
    //: func refreshGift(_ model: SocialBroadcastModel) {
    func drownTarget(_ model: MarkHandyJSON) {
        //: giftModel = model
        giftModel = model
        //: icon.setUrlImage(urlStr: giftModel.headPic)
        icon.sitImageDoing(urlStr: giftModel.headPic)
        ...
        if giftModel.broadStatus == 1 { // 中奖
            ...
            contentLab.text = (String(str_pageName.prefix(4))).localized + "\(giftModel.totalCoin)" + (String(str_replyName.prefix(2)) + str_blockContent.replacingOccurrences(of: "action", with: "n") + " in ").localized + giftModel.gameName
            ...
            giftImgV.sitImageDoing(urlStr: giftModel.img)
            //: giftNumLab.isHidden = false
            giftNumLab.isHidden = false
            //: giftNumLab.text = " x\(giftModel.num)"
            giftNumLab.text = " x\(giftModel.num)"
            //: let rangeText = "just sent".localized
            let rangeText = (String(str_playAppValue)).localized
            //: let giftName = changeGiftName(giftmodel: giftModel)
            let giftName = dorsalVertebra(giftmodel: giftModel)
            ...
            if let range = fullText.range(of: rangeText) {
                ...
                attributedString.addAttribute(.foregroundColor, value: UIColor(hex: (String(str_layerData.prefix(7))))!, range: nsRange)
            }
    ...
    private func dorsalVertebra(giftmodel: MarkHandyJSON) -> String {
...
extension MemoryFloatView {
    ...
    private func pushSubviews() {
        ...
        self.frame = CGRect(x: 0, y: kLet_timeScreenValue, width: kLet_itemData, height: 60)
